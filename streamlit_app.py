...
def _table_qc_issues(d, table_type, label):
    ...
    if table_type == "taux":
        ...
        for k, v in d.items():
            ...
            if prev is not None and isinstance(prev, (int,float)):
                ...
                # Grand saut en pp (variation absolue)
                    ...
                    if abs(delta_pp) > 3 and abs(delta_rel) > 50:
        ...
        for site, sd in site_data.items():
            ...
            # Site > dédup (market share > 100%)
            if last and dedup and isinstance(dedup, (int,float)) and dedup > 100:
                ms = last / dedup * 100
                if ms > 100.5:
                    issues.append({
                        "type": "MS_OVER_100",
                        "severity": "error",
                        "site": site,
                        "message": f"Annonces site ({_fmtn(last)}) > Total dédupliqué ({_fmtn(dedup)}) → share {ms:.0f}%",
                        "values": f"{site}: {_fmtn(last)} annonces / {_fmtn(dedup)} dédup = {ms:.0f}%",
                    })
    ...
    with sub2:
        ...
        if not issues:
            ...
            ISSUE_EXPLANATIONS = {
                ...
                "MS_OVER_100":    "Site listings > Deduplicated total — deduplication is inconsistent.",
