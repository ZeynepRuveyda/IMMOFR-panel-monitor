...
def analyse_all_tables(raw_bytes_dict):
    results=[]
    fb=classify_named(raw_bytes_dict)
    SKIP_={"Total","Total Panel Dédupliqué","Total Panel Dédupliqué - Top 5 Sites",
           "Total Panel Dédupliqué  - Top 11 Sites","Total Panel Dédupliqué Marché",
           "Immobilier Notaire","Immonot","Site","Département","Totaux","TOTAL"}
...
with tab_dq:
    ...
    with tab_form:
        ...
        rows_exp_f=[{"Category":GROUP_INFO.get(c["group"],("",""))[0],"Check":c["name"],
    ...
    with tab_tables:
        if not tables_show:
            ...
            # Full-name overview — every file / tab / table is listed here in plain text,
            # nothing hidden behind generic category labels.
            st.markdown("##### 📋 All files, tabs & tables checked")
            overview_rows=[{
                "": "❌" if t["n_error"] else ("🟠" if (t["n_warn"] and any(i.get("type") in SURGE_TYPES for i in t["issues"])
                        and all(i.get("type") in SURGE_TYPES for i in t["issues"] if i["severity"]=="warning")) else "⚠️") if t["n_warn"] else "✅",
                "File": t.get("source_file") or t["file"],
                "Tab (sheet)": t["sheet"],
                "Table / section": t["label"] or f"Section {t['sec_idx']+1}",
                "Type": TTYPE_LBL.get(t["table_type"], t["table_type"]).split(" — ")[0],
                "Month": t["lm"],
                "Issues": len(t["issues"]),
            } for t in sorted(tables_show, key=lambda t:(0 if t["n_error"] else 1 if t["n_warn"] else 2,
                                                           t.get("source_file") or t["file"], t["sheet"], t["sec_idx"]))]
            df_overview = pd.DataFrame(overview_rows)
            def _cr_ov(df):
                def _row(row):
                    s=str(row.get("",""))
                    bg="#fdecec" if s=="❌" else "#fff3e0" if s=="🟠" else "#fff8e1" if s=="⚠️" else "#edf7ed"
                    return [f"background-color:{bg}" for _ in row]
                return df.style.apply(_row,axis=1)
            st.dataframe(_cr_ov(df_overview), use_container_width=True, hide_index=True,
                         height=min(600, 42+35*len(df_overview)),
                         column_config={
                             "File": st.column_config.TextColumn("File", width="large"),
                             "Tab (sheet)": st.column_config.TextColumn("Tab (sheet)", width="medium"),
                             "Table / section": st.column_config.TextColumn("Table / section", width="large"),
                         })
            ...
            file_options = sorted(set(t.get("source_file") or t["file"] for t in tables_show))
            def _file_badge(f):
                ts=[t for t in tables_show if (t.get("source_file") or t["file"])==f]
            ...
            file_tables = [t for t in tables_show if (t.get("source_file") or t["file"])==sel_file]
