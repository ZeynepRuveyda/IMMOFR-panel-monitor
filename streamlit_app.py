        ...
        try:
            ...
            if ws11:
                ...
                c5.metric("Trend alerts (M/M-1)",
                          f"🔴 {n_alert} · 🟠 {n_surge} · 🟡 {n_wtr}",
                          help="Critical, surge and warning series at website level only. Dept/région trends visible in Trends tab.")
            wb1.close()
        except: pass
...
with tab_dq:
    ...
    with tab_tables:
        if not tables_show:
            ...
            def _table_status(t):
                if t["n_error"]>0: return "❌ Error"
                if t["n_warn"]>0:
                    warn_issues=[i for i in t["issues"] if i["severity"]=="warning"]
                    if warn_issues and all(i.get("type") in SURGE_TYPES for i in warn_issues):
                        return "🟠 Surge"
                    return "🟡 Warning"
                return "✅ Clean"
            ...
            tables_f=[t for t in tables_show if _table_status(t) in status_filter] if status_filter else []
            ...
            if not tables_f:
                ...
                # ── Full-name overview, collapsed — every file / tab / table checked,
                #    respecting the same colour filter selected above ──
                with st.expander(f"📋 All files, tabs & tables checked ({len(tables_f)} shown)", expanded=False):
                    overview_rows=[{
                        "": _table_status(t)[0],
                        "File": t.get("source_file") or t["file"],
                        "Tab (sheet)": t["sheet"],
                        "Table / section": t["label"] or f"Section {t['sec_idx']+1}",
                    } for t in sorted(tables_f, key=lambda t:(0 if t["n_error"] else 1 if t["n_warn"] else 2,
