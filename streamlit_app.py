...
def read_series_generic(ws):
    ...
    ws = ws_get(wbs["file1"], "Total") or ws_get(wbs["file1"], "1.1 Total")
    if not ws: return C
    d = read_series(ws, section=0)
    if not d: return C
    lm = d.get("_lm", "?")
    total = sv(d, "Total")
    dedup = next((sv(d, k) for k in d if isinstance(k,str) and "dédup" in k.lower() and not k.startswith("_")), None)
    if total and dedup:
        C.append(chk(f"Dedup ≤ Total ({lm})", dedup<=total*1.01, f"Dedup: {_fmt_full(dedup)} · Total: {_fmt_full(total)}", "1"))
    for site in sites:
        v = sv(d, site)
        if v is not None:
            C.append(chk(f"{site} — data present ({lm})", v > 0, f"{site}: {_fmt_full(v)}", "1"))
    return C
