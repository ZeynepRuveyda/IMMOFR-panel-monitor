...
def read_series(ws, col=2, section=0):
    ...
    for c in range(col+1,ws.max_column+1):
        ...
        if key is not None and last_key is not None and key <= last_key:
            # Date goes backward/repeats — a second segment block (e.g. "Particuliers" then
            # "Professionnels" columns reusing the same 2 months) starts here, not a real
            # continuation of the time series. Stop, or later M/M-1 math silently mixes two
            # unrelated segments into one fake "evolution" (a real bug found in AUTOFR's
            # 'Evolution' sheet, where the two segments got read as 4 sequential months).
            break
        if key is not None:
            last_key = key
...
def build_trends(raw_bytes_dict, wbs=None):
    ...
    for fname, data in raw_bytes_dict.items():
        try:
            ...
            continue
        for sn in wb.sheetnames:
            if sn == "Intro" or sn.strip().lower() == "evolution":
                # "Evolution" (AUTOFR file1) is a 2-month M-vs-M-1 snapshot that duplicates
                # the same sites already covered — with full history — by Total/Particulier/
                # Pros/Groupes in the same file. Keeping it produces a redundant second card
                # for the exact same anomaly, with a far less informative 2-point chart.
                continue
            ws = wb[sn]
...
def analyse_all_tables(raw_bytes_dict, classify_fn=None, file_roles=None):
    ...
    for role,(source_file,data) in fb.items():
        ...
        for sn in wb.sheetnames:
            if sn=="Intro" or sn.strip().lower()=="evolution": continue
            ws=wb[sn]
