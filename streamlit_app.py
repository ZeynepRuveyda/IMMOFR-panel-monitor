...
def read_series(ws, col=2, section=0, run_idx=0):
    ...
    """
    hdrs=[r for r in range(1,ws.max_row+1) if ws.cell(r,col).value in ("Site","Département","Région","Website")]
    ...
    # Section label (row above header) — prefer the segment's own label found just
    # above its first data column (e.g. "Particuliers"/"Professionnels" sitting side
    # by side after one 'Site' header); fall back to the generic sheet-level title
    # at `col` if nothing more specific is found there.
    label=None
    for tr in range(hdr-1,max(0,hdr-5),-1):
        lv=ws.cell(tr,mc[0]).value
...
def sheet_all_segments_for_site(raw_bytes_dict, fname, sheet_name, site):
    """
    Re-read one sheet and return EVERY segment's data for one site — used to combine
    sibling segments (e.g. AUTOFR 'Evolution' sheet's 'Particuliers' + 'Professionnels',
    which sit side by side after the same header row — see _month_runs) into a single
    comparison chart, instead of two separate, confusing cards for the same site.
    Returns a list of {"label", "months", "vals"} dicts, one per segment that has data
    for this site (segments where the site is absent/zero-only are skipped).
    """
    data = raw_bytes_dict.get(fname)
    if not data:
        return []
    try:
        wb = load_wb_any(data)
...
def read_cross(ws,col=2):
    ...
    for r in range(1,min(20,ws.max_row+1)):
        if ws.cell(r,col).value in ("Site","Département","Région","Website"): hdr=r; break
    if not hdr:
        for r in range(1,min(20,ws.max_row+1)):
            if ws.cell(r,1).value in ("Site","Département","Région","Website"): hdr=r; col=1; break
    if not hdr: return {}
    sites,scols=[],[]
    for c in range(col+1,ws.max_column+1):
        ...
        with cols3[i % 3]:
            ...
            if n_real <= 2:
                ...
                if segments and len(segments) >= 2:
                    # Multiple sibling segments for the same site+sheet (e.g. AUTOFR
                    # 'Evolution' sheet's 'Particuliers' + 'Professionnels', which sit
                    # side by side after the same header row) — show them TOGETHER in
                    # one grouped bar chart (segment × month) instead of separate,
                    # confusing single-segment cards.
                    palette = ["#4a90d9", "#ff8c00", "#8e44ad", "#16a085"]
                    fig = go.Figure()
                    for si, seg in enumerate(segments[:4]):
                        seg_vals = seg["vals"]
                        seg_months = seg["months"][:len(seg_vals)]
                        seg_name = seg["label"] or f"Segment {si+1}"
                        fig.add_trace(go.Bar(
                            x=seg_months, y=seg_vals, name=seg_name,
                            marker_color=palette[si % len(palette)],
                            text=[f"{v:,.0f}" if v is not None else "" for v in seg_vals],
                            textposition="outside",
                            hovertemplate="%{x}: %{y:,.0f}<extra>"+seg_name+"</extra>"))
                    ...
                    # Only 2 real months, single segment — a line is misleading (barely a
                    # "trend"), and with a sparse x-axis it can even look like a zigzag. A
                    # simple M-1→M bar comparison reads correctly at a glance for 2 points.
                    bar_x = x_vals[-2:] if len(x_vals) >= 2 else x_vals
                    bar_y = vals[-2:] if len(vals) >= 2 else vals
                    while len(bar_x) < 2: bar_x = ["—"] + bar_x
                    while len(bar_y) < 2: bar_y = [None] + bar_y
                    fig = go.Figure(go.Bar(
                        x=list(bar_x), y=list(bar_y), marker_color=clr, width=[0.5,0.5],
            ...
            st.plotly_chart(fig, use_container_width=True,
                            ...
                            key=f"{key_prefix}_{i}_{label[:6]}_{n_real}_{APP_VERSION.replace('.','')}")
...
def analyse_all_tables(raw_bytes_dict, classify_fn=None, file_roles=None):
    ...
    SKIP_=SKIP
    for role, val in fb.items():
        # classify_named returns (source_file, data), classify_generic returns data directly
        if isinstance(val, tuple):
            source_file, data = val
        else:
            source_file, data = role, val
        fname=file_roles.get(role,role)
        try: wb=load_wb_any(data)
        except: continue
        for sn in wb.sheetnames:
            if sn=="Intro": continue
            ws=wb[sn]
            ...
            for c in [2, 1]:
                hc=sum(1 for r in range(1,min(ws.max_row+1,30))
                       if ws.cell(r,c).value in ("Site","Département","Région","Website"))
                if hc>0: col=c; break
            hdr_count=sum(1 for r in range(1,ws.max_row+1)
                          if ws.cell(r,col).value in ("Site","Département","Région","Website"))
...
MARKET_CONFIG = {
    "IMMOFR":   {"key":"IMMOFR",   "label":"IMMO FR",   "emoji":"🏠", "sites":IMMOFR_SITES,  "file_roles":FILE_ROLES,         "group_info":GROUP_INFO,         "classify_fn":None,      "run_checks_fn":None,      "has_special_check":True,  "has_dept_region":True},
    "AUTOFR":   {"key":"AUTOFR",   "label":"AUTO FR",   "emoji":"🚗", "sites":AUTOFR_SITES,  "file_roles":FILE_ROLES_AUTOFR,  "group_info":GROUP_INFO_AUTOFR,  "classify_fn":"autofr",  "run_checks_fn":"autofr",  "has_special_check":False, "has_dept_region":False},
    "IMMOUK":   {"key":"IMMOUK",   "label":"IMMO UK",   "emoji":"🏠", "sites":["Rightmove","Zoopla","OnTheMarket","PrimeLocation","Loopnet"], "file_roles":{}, "group_info":{}, "classify_fn":"generic","run_checks_fn":"generic","has_special_check":False,"has_dept_region":False},
    "IMMODE":   {"key":"IMMODE",   "label":"IMMO DE",   "emoji":"🏠", "sites":["Immobilienscout24","Immowelt.De","Ebay Kleinanzeigen"], "file_roles":{}, "group_info":{}, "classify_fn":"generic","run_checks_fn":"generic","has_special_check":False,"has_dept_region":False},
    "AUTOUK":   {"key":"AUTOUK",   "label":"AUTO UK",   "emoji":"🚗", "sites":["AutoTrader","CarGurus","Cazoo","eBay","Carwow","CarAndClassic","Cinch","ExchangeAndMart"], "file_roles":{"file1":"1 — Website offers panel","file2":"2 — Offers panel","file3":"3 — Performance panel","file4":"4 — Clients panel","file5":"5 — Pivot doubles"}, "group_info":{}, "classify_fn":"generic","run_checks_fn":"generic","has_special_check":False,"has_dept_region":False},
    "EMPLOIFR": {"key":"EMPLOIFR", "label":"EMPLOI FR", "emoji":"📰", "sites":["Apec","Cadremploi","Figaro Emploi","HelloWork","Indeed","Jobintree","Jobteaser","Leboncoin","Meteojob","Monster","Ouestfrance","France Travail","Talent","Welcometothejungle"], "file_roles":{"file1":"1 — Panel evolution","file2":"2 — Performance","file3":"3 — Quality","file4":"4 — Annonceurs","file5":"5 — Doublons croisés","file6":"6 — Stats départementales","file7":"7 — Stats régionales"}, "group_info":{}, "classify_fn":"generic","run_checks_fn":"generic","has_special_check":False,"has_dept_region":False},
    "TRUCKSFR": {"key":"TRUCKSFR", "label":"TRUCKS-AGRO", "emoji":"🚛", "sites":["autoline24.fr","europe-camions.com","leboncoin.fr","mascus.fr","truckplus.fr","truckscorner.fr","agriaffaires.fr","placedestracteurs.fr","terre-net-occasions.fr","tracteurpool.fr","baupool.fr","europe-tp.com","machineryzone.fr","tp-business.fr"], "file_roles":{"file1":"1 — Website offers panel","file2":"2 — Offers panel","file3":"3 — Performance panel","file4":"4 — Clients panel","file5":"5 — Countries panel","file6":"6 — Pivot doubles"}, "group_info":{}, "classify_fn":"generic","run_checks_fn":"generic","has_special_check":False,"has_dept_region":False},
    "AUTOBR":   {"key":"AUTOBR",   "label":"AUTO BR",   "emoji":"🚗", "sites":["Autoline","Carropiracicaba","Comprecar","Icarros","Mobiauto","Napista","OLX","Seminovosbh","Socarrao","Usadosbr","Webmotors"], "file_roles":{"file1":"1 — Website offers panel","file2":"2 — Offers panel","file3":"3 — Performance panel","file4":"4 — Clients panel","file5":"5 — Countries panel"}, "group_info":{}, "classify_fn":"generic","run_checks_fn":"generic","has_special_check":False,"has_dept_region":False},
    "NPV":      {"key":"NPV",      "label":"NPV UK",    "emoji":"🏠", "sites":["AutoTrader","Cazoo","eBay","Gumtree","MotorcyclesNews","Motors Group","CaravanFinder","Cargurus","Motors"], "file_roles":{"file1":"1 — Website offers panel","file2":"2 — Offers panel","file3":"3 — Performance panel","file4":"4 — Clients panel","file4bis":"4bis — Clients bis","file5":"5 — Provinces"}, "group_info":{}, "classify_fn":"generic","run_checks_fn":"generic","has_special_check":False,"has_dept_region":False},
    "IMMOLU":   {"key":"IMMOLU",   "label":"IMMO LU",   "emoji":"🏠",
                 "sites":["Athome","Immotop","Logic-immo","Era","Wortimmo","Wort"],
                 "file_roles":{"file1":"1 — Websites Analysis","file2":"2 — Performance Analysis",
                               "file3":"3 — Agent Analysis","file4":"4 — Regional Analysis","file5":"5 — Shared Analysis"},
                 "group_info":{"1":("Websites Analysis","Total ads, private/agents, sale/rent"),"2":("Performance Analysis","Quality metrics"),
                               "3":("Agent Analysis","Pro breakdown"),"4":("Regional Analysis","Regional stats"),"5":("Shared Analysis","Shared listings")},
                 "classify_fn":"lu","run_checks_fn":"generic","has_special_check":False,"has_dept_region":False},
    "AUTOLU":   {"key":"AUTOLU",   "label":"AUTO LU",   "emoji":"🚗",
                 "sites":["Autoscout24","Luxauto","Mobile","Cardosso","Motortrend"],
                 "file_roles":{"file1":"1 — Websites Analysis","file2":"2 — Offer Analysis","file3":"3 — Performance Analysis",
                               "file4":"4 — Dealers Analysis","file5":"5 — Regional Analysis","file6":"6 — Shared Analysis"},
                 "group_info":{"1":("Websites Analysis","Total ads evolution"),"2":("Offer Analysis","Offer breakdown"),
                               "3":("Performance Analysis","Quality metrics"),"4":("Dealers Analysis","Dealer stats"),
                               "5":("Regional Analysis","Regional stats"),"6":("Shared Analysis","Shared listings")},
                 "classify_fn":"lu","run_checks_fn":"generic","has_special_check":False,"has_dept_region":False},
}

def classify_lu(raw, file_roles=None):
    """Classify IMMOLU/AUTOLU files by name pattern."""
    out = {}
    for fname, data in raw.items():
        nl = norm(fname)
        if "1_website" in nl:
            out.setdefault("file1", data)
        elif "2_offer" in nl:
            out.setdefault("file2", data)
        elif "2_perform" in nl:
            out.setdefault("file2", data)
        elif "3_agent" in nl:
            out.setdefault("file3", data)
        elif "3_perform" in nl:
            out.setdefault("file3", data)
        elif "4_region" in nl:
            out.setdefault("file4", data)
        elif "4_dealer" in nl:
            out.setdefault("file4", data)
        elif "5_shared" in nl:
            out.setdefault("file5", data)
        elif "5_region" in nl:
            out.setdefault("file5", data)
        elif "6_shared" in nl:
            out.setdefault("file6", data)
    return out

def load_wb_any(data):
    """Load workbook from bytes — supports .xlsx and .xls (OLE format)."""
    try:
            ...
            for r in range(ws_xls.nrows):
                for c in range(ws_xls.ncols):
                    ...
                    if ctype == xlrd.XL_CELL_DATE:
                        try:
                            dt = xlrd.xldate_as_datetime(cell.value, xls.datemode)
                        ...
                        ws.cell(row=r+1, column=c+1, value=bool(cell.value))
        return wb
    except Exception as e:
        raise ValueError(f"Cannot open file: {e}")

def classify_generic(raw, file_roles=None):
    """Generic classify — assign files by upload order. Auto-generates roles if file_roles empty."""
    files = list(raw.items())
    if not file_roles:
        file_roles = {f"file{i+1}": f"File {i+1}" for i in range(len(files))}
    roles = list(file_roles.keys())
    return {roles[i]: data for i, (fname, data) in enumerate(files) if i < len(roles)}

def read_series_generic(ws):
    """
    Market-agnostic time series reader.
    Handles 3 layouts:
    1. Standard: Site/Website col, month cols right (IMMOFR style)
    2. Paired:   Websites col, month pairs (Listings+Share) right (AUTOLU style)  
    3. Transposed: Sites as column headers, metrics as rows (IMMOLU file2 style)
    Returns: {site: {last, prev}, _lm, _pm}
    """
    import datetime as _dt
    max_r = min(ws.max_row+1, 200)
    max_c = min(ws.max_column+1, 100)

    # ── Find date row (row with datetime values) ──
    date_row = None; date_cols = []
    for r in range(1, 20):
        dcs = [(c, ws.cell(r,c).value) for c in range(1,max_c)
               if isinstance(ws.cell(r,c).value, _dt.datetime)]
        if len(dcs) >= 2:
            date_row = r; date_cols = dcs; break

    if not date_row:
        return {}

    # ── Find site column (col with known header keywords) ──
    SITE_HDRS = ("Site","Websites","Website","Département","Région","sites")
    site_col = None; site_row = None
    for r in range(1, date_row+3):
        for c in range(1, 6):
            v = ws.cell(r,c).value
            if isinstance(v,str) and v.strip() in SITE_HDRS:
                site_col = c; site_row = r; break
        if site_col: break

    # Layout 1 & 2: site names in column, months in row
    if site_col:
        # For paired layout: find "Listings" sub-header row
        listing_col_map = {}  # date → col with listings
        sub_row = date_row + 1
        sub_vals = [(c, ws.cell(sub_row,c).value) for c in range(1,max_c)
                    if ws.cell(sub_row,c).value]
        if any(isinstance(v,str) and "listing" in str(v).lower() for _,v in sub_vals):
            # Paired: map each date to its listings column
            for dc, dv in date_cols:
                # look for "Listings" in same col or next col
                for offset in range(0,3):
                    sv2 = ws.cell(sub_row, dc+offset).value
                    if isinstance(sv2,str) and "listing" in sv2.lower():
                        listing_col_map[dc] = dc+offset; break
            data_start_row = sub_row + 1
            lc = listing_col_map.get(date_cols[-1][0], date_cols[-1][0])
            pc = listing_col_map.get(date_cols[-2][0], date_cols[-2][0])
        else:
            # Standard: dates directly map to value cols
            lc = date_cols[-1][0]; pc = date_cols[-2][0]
            data_start_row = date_row + 1

        out = {"_lm": date_cols[-1][1].strftime("%b-%y"),
               "_pm": date_cols[-2][1].strftime("%b-%y")}
        for r in range(data_start_row, max_r):
            site = ws.cell(r, site_col).value
            if not site or not isinstance(site, str) or not site.strip(): continue
            site = site.strip()
            lv = ws.cell(r, lc).value
            pv = ws.cell(r, pc).value
            if isinstance(lv,(int,float)) and isinstance(pv,(int,float)):
                out[site] = {"last": lv, "prev": pv}
        return out

    # Layout 3: transposed — sites as col headers
    # Find site header row (row where cell values are site names)
    SKIP_VALS = {"Number","%","Listings","Market share","","None"}
    site_header_row = None; site_col_map = {}
    for r in range(1, date_row+5):
        row_vals = {c: ws.cell(r,c).value for c in range(2,max_c)
                    if ws.cell(r,c).value and str(ws.cell(r,c).value) not in SKIP_VALS}
        if len(row_vals) >= 2 and all(isinstance(v,str) for v in row_vals.values()):
            # Could be site header row — check if col below has numbers
            next_row_nums = sum(1 for c in row_vals if isinstance(ws.cell(r+2,c).value,(int,float)))
            if next_row_nums >= 1:
                site_header_row = r
                site_col_map = {v.strip(): c for c,v in row_vals.items()}
                break

    if site_header_row and len(site_col_map) >= 2:
        # Find the metric row for the reference month
        # date_cols gives us which cols have the dates — but in transposed layout
        # dates are not in a row. Just return summary stats from site cols.
        # Look for a "total" or main metric row
        metric_rows = []
        for r in range(site_header_row+1, min(site_header_row+20, max_r)):
            label = ws.cell(r, 1).value or ws.cell(r, 2).value
            if not label: continue
            # Pick rows with mostly numeric values
            nums = [ws.cell(r,c).value for c in site_col_map.values()
                    if isinstance(ws.cell(r,c).value,(int,float))]
            if nums and max(nums) > 100:
                metric_rows.append((r, str(label).strip(), nums))
        if metric_rows:
            # Use first substantial metric row as "last" — can't do M/M-1 in transposed
            r, label, _ = metric_rows[0]
            out = {"_lm":"?", "_pm":"?"}
            for site, c in site_col_map.items():
                v = ws.cell(r,c).value
                if isinstance(v,(int,float)):
                    out[site] = {"last": v, "prev": v}  # no prev in transposed
            return out

    return {}

def read_long_format(ws):
    """
    Read long-format panel files (Country/Year/Month/Website/KPI/Value).
    Returns {site: {last, prev}, _lm, _pm} with prev=last (single month).
    Supports:
    - IMMODE: col1=Country, col2=Year, col3=Month, col4=Website, col5=KPI, col6=Value
    - IMMOUK: col1=Year, col2=Month, col3=Website, col4=Type, col8=Value
    """
    if ws.max_row < 2: return {}
    # Detect format from headers
    h = [ws.cell(1,c).value for c in range(1, min(ws.max_column+1, 10))]
    h_lower = [str(v).lower() if v else "" for v in h]

    # IMMODE format: Country,Year,Month,Website,KPI,Value
    if "country" in h_lower and "kpi" in h_lower:
        site_col = next((i+1 for i,v in enumerate(h_lower) if "website" in v), 4)
        kpi_col  = next((i+1 for i,v in enumerate(h_lower) if "kpi" in v), 5)
        val_col  = next((i+1 for i,v in enumerate(h_lower) if "value" in v), 6)
        yr_col   = next((i+1 for i,v in enumerate(h_lower) if "year" in v), 2)
        mo_col   = next((i+1 for i,v in enumerate(h_lower) if "month" in v), 3)
        TARGET_KPIS = {"total listings","listings","pro listings","total ads",
                       "exclusive listings","sales listings","sales dedup listings",
                       "dedup listings","total dedup"}
        TOTAL_KPIS = {"total listings","listings","total ads"}
        DEDUP_KPIS = {"sales dedup listings","dedup listings","total dedup"}
        out = {}; lm = None
        for r in range(2, ws.max_row+1):
            site = ws.cell(r, site_col).value
            kpi  = str(ws.cell(r, kpi_col).value or "").lower()
            val  = ws.cell(r, val_col).value
            yr   = ws.cell(r, yr_col).value
            mo   = ws.cell(r, mo_col).value
            if not site or not isinstance(val, (int,float)): continue
            if not any(t in kpi for t in TARGET_KPIS): continue
            if not lm and yr and mo:
                import datetime
                try: lm = datetime.datetime(int(yr),int(mo),1).strftime("%b-%y")
                except: lm = f"{int(mo)}/{int(yr)}"
            # MARKET → aggregate totals
            if site.upper() in ("MARKET","TOTAL"):
                if any(t in kpi for t in DEDUP_KPIS):
                    out.setdefault("Deduplicated total", {"last": val, "prev": val})
                elif any(t in kpi for t in TOTAL_KPIS):
                    out.setdefault("Total", {"last": val, "prev": val})
            else:
                out.setdefault(site.title(), {"last": val, "prev": val})
        out["_lm"] = lm or "?"; out["_pm"] = "N/A"
        return out

    # IMMOUK format: Year,Month,Website,Type,Category,...,Value
    if "website" in h_lower and "type" in h_lower:
        site_col = next((i+1 for i,v in enumerate(h_lower) if "website" in v), 3)
        type_col = next((i+1 for i,v in enumerate(h_lower) if "type" in v), 4)
        cat_col  = next((i+1 for i,v in enumerate(h_lower) if "category" in v), 5)
        yr_col   = next((i+1 for i,v in enumerate(h_lower) if "year" in v), 1)
        mo_col   = next((i+1 for i,v in enumerate(h_lower) if "month" in v), 2)
        val_col  = max((i+1 for i,v in enumerate(h_lower)
                       if v and any(x in v for x in ["aout","août","value","total","listings"])
                       and i+1 > type_col), default=8)
        out = {}; lm = None
        for r in range(2, ws.max_row+1):
            site = ws.cell(r, site_col).value
            typ  = str(ws.cell(r, type_col).value or "").lower()
            cat  = str(ws.cell(r, cat_col).value or "")
            val  = ws.cell(r, val_col).value
            yr   = ws.cell(r, yr_col).value
            mo   = ws.cell(r, mo_col).value
            if not isinstance(val, (int,float)): continue
            if cat not in ("-","","None",None): continue  # top-level only
            if not lm and yr and mo:
                import datetime
                try: lm = datetime.datetime(int(yr),int(mo),1).strftime("%b-%y")
                except: lm = f"{mo}/{yr}"
            # Market rows → total & dedup
            if str(site or "").strip().lower() == "market":
                if "dedup" in typ:
                    out.setdefault("Deduplicated total", {"last": val, "prev": val})
                elif "listing" in typ:
                    out.setdefault("Total", {"last": val, "prev": val})
            # Site rows → listings
            elif site and "listing" in typ:
                out.setdefault(str(site), {"last": val, "prev": val})
        out["_lm"] = lm or "?"; out["_pm"] = "N/A"
        return out

    return {}

def read_long_format_all(raw_bytes_dict):
    """
    Read multiple single-month long-format files and combine into time series.
    Returns {site: {last, prev, vals, months}, _lm, _pm} for M/M-1 comparison.
    Works for IMMOUK (Year/Month/Website/Type) and IMMODE (Country/Year/Month/Website/KPI/Value).
    """
    import datetime as _dt

    # Read each file → {month_str: {site: value}}
    monthly_data = {}  # {month_str: {site: val, "Total": val, "Dedup": val}}
    ...
    for fname, data in raw_bytes_dict.items():
        try:
            ...
            continue
        for sn in wb.sheetnames:
            if sn.lower() in ("intro","feuil1","sheet1","readme","cover"): continue
            ws = wb[sn]
            d = read_long_format(ws)
            if not d or not d.get("_lm") or d["_lm"] == "?": continue
            lm = d["_lm"]
            if lm not in monthly_data:
                monthly_data[lm] = {}
            for k, v in d.items():
                if not k.startswith("_"):
                    monthly_data[lm][k] = v["last"]
            break  # first valid sheet per file
        wb.close()
    ...
    if len(sorted_months) == 1:
        # Single month — no M/M-1
        lm = sorted_months[0]
        sites = [k for k in monthly_data[lm] if k not in ("Total","Deduplicated total")]
        rows = []
        for site in sites:
            val = monthly_data[lm].get(site, 0)
            rows.append({
                "site": site, "sheet": "Panel", "section": "",
                "lm": lm, "pm": "N/A",
                "lv": val, "pv": None, "evol": None, "evol_y1": None,
                "months": [lm], "vals": [val],
                "prm": None, "status": "ok", "flags": [],
            })
        return rows

    # Multiple months — build M/M-1
    rows = []
    lm = sorted_months[-1]
    pm = sorted_months[-2]
    all_sites = set()
    for m in sorted_months:
        all_sites.update(k for k in monthly_data[m]
                         if k not in ("Total","Deduplicated total"))

    for site in sorted(all_sites):
        vals = [monthly_data.get(m, {}).get(site) for m in sorted_months]
        lv = monthly_data.get(lm, {}).get(site)
        pv = monthly_data.get(pm, {}).get(site)
        evol = round((lv/pv-1)*100, 6) if lv and pv and pv > 0 else None
        status, flags = classify_mom_status(vals, len(sorted_months)-1)
        rows.append({
            "site": site, "sheet": "Panel", "section": "",
            "lm": lm, "pm": pm,
            "lv": lv, "pv": pv, "evol": evol, "evol_y1": None,
            "months": sorted_months, "vals": vals,
            "prm": None, "status": status, "flags": flags,
        })
    return rows

def run_checks_generic(fb, wbs, cfg):
    """Generic run_checks — basic structural checks for any market."""
    C = []; sites = cfg.get("sites", [])
    if "file1" not in wbs: return C
    ws_tot = (ws_get(wbs["file1"], "Total") or ws_get(wbs["file1"], "1.1 Total Ads") or
              ws_get(wbs["file1"], "1.1 Total") or wbs["file1"].active)
    if not ws_tot: return C
    d = read_series(ws_tot, section=0)
    if not d or not d.get("_lm"):
        d = read_series_generic(ws_tot)
    if not d or not d.get("_lm"):
        d = read_long_format(ws_tot)
    if not d: return C
    lm = d.get("_lm", "?")
    total = sv(d, "Total") or sv(d, "Somme Panel") or sv(d, "Total Panel")
    dedup = next((sv(d, k) for k in d if isinstance(k,str) and
                  ("dédup" in k.lower() or "dedup" in k.lower()) and not k.startswith("_")), None)
    if total and dedup:
        C.append(chk(f"Dedup ≤ Total ({lm})", dedup<=total*1.01,
                     f"Dedup: {fmt(dedup)} · Total: {fmt(total)}", "1"))
    for site in sites:
        v = sv(d, site)
        if v is not None:
            C.append(chk(f"{site} — data present ({lm})", v > 0, f"{site}: {fmt(v)}", "1"))
    return C

@st.cache_data(show_spinner="Running quality checks…")
def compute_everything_generic(file_hash, raw_bytes_dict, market_key, _cache_bust_version=APP_VERSION):
    """Generic compute pipeline — works for any market in MARKET_CONFIG."""
    cfg = next(v for v in MARKET_CONFIG.values() if v["key"] == market_key)
    global SITES
    SITES[:] = cfg["sites"]

    if cfg["classify_fn"] == "autofr":
        fb_named = classify_named_autofr(raw_bytes_dict)
        fb = {role: data for role, (fn, data) in fb_named.items()}
    elif cfg["classify_fn"] == "lu":
        fb = classify_lu(raw_bytes_dict, cfg["file_roles"])
    elif cfg["classify_fn"] == "generic":
        fb = classify_generic(raw_bytes_dict, cfg["file_roles"])
    else:
        fb = classify(raw_bytes_dict)
    ...
    if cfg["run_checks_fn"] == "autofr":
        checks = run_checks_autofr(fb, wbs)
    elif cfg["run_checks_fn"] == "generic":
        checks = run_checks_generic(fb, wbs, cfg)
    else:
        checks = run_checks(fb, wbs)

    trends = build_trends(raw_bytes_dict, wbs)
    # For long-format markets (IMMOUK, IMMODE), build_trends may return empty or
    # only aggregate rows — check if we have real site-level data
    _real_sites = set(r["site"] for r in trends if r["site"] not in
                      ("Total","Total Panel","Deduplicated total","TOTAL","MARKET"))
    _market_sites = set(cfg.get("sites", []))
    if not trends or (_market_sites and len(_real_sites & _market_sites) == 0):
        trends = read_long_format_all(raw_bytes_dict)
    file_roles = cfg["file_roles"]
    tables = analyse_all_tables(
        raw_bytes_dict,
        classify_fn=(lambda r: classify_generic(r, file_roles)) if cfg["classify_fn"]=="generic" else None,
        file_roles=file_roles
    )
    mshare = market_share_analysis(wbs) if cfg["has_special_check"] else {}
    return checks, trends, fb, mshare, tables
...
# Initialize before sidebar — will be overwritten inside
uploaded = None
raw_bytes = {}
file_hash = None
market_cfg = MARKET_CONFIG["IMMOFR"]
market = "IMMOFR"
SITES[:] = IMMOFR_SITES
...
with st.sidebar:
    ...
    _market_choice = st.selectbox("Market", list(MARKET_CONFIG.keys()),
                                   format_func=lambda k: f"{MARKET_CONFIG[k]['emoji']} {MARKET_CONFIG[k]['label']}",
                                   label_visibility="collapsed", key="market_choice")
    market_cfg = MARKET_CONFIG[_market_choice]
    market = market_cfg["key"]
    SITES[:] = market_cfg["sites"]
    st.divider()

    _up_key = f"up_{market.lower()}"
    uploaded = st.file_uploader("Upload all Excel files", type=["xlsx","xls"],
                                 accept_multiple_files=True, key=_up_key)
    ...
    if uploaded:
        ...
        file_roles = market_cfg["file_roles"]

        if market_cfg["classify_fn"] == "autofr":
            fb_display = classify_named_autofr(raw_bytes)
        elif market_cfg["classify_fn"] == "lu":
            fb_display = classify_lu(raw_bytes, file_roles)
        elif market_cfg["classify_fn"] == "generic":
            fb_display = classify_generic(raw_bytes, file_roles or None)
        else:
            fb_display = classify(raw_bytes)

        n_ok_files = len(fb_display)

        # For generic markets with empty file_roles, show actual filenames
        if market_cfg["classify_fn"] == "generic" and not file_roles:
            st.success(f"✅ {n_ok_files} file{'s' if n_ok_files>1 else ''} loaded")
            for i, fname in enumerate(raw_bytes.keys()):
                st.caption(f"✅ File {i+1} — {fname[:35]}")
        else:
            n_req = len(file_roles) if file_roles else n_ok_files
            if n_ok_files >= n_req:
                st.success(f"✅ All {n_ok_files} files loaded")
            elif n_ok_files >= 1:
                st.info(f"📁 {n_ok_files} / {n_req} files loaded")
            else:
                st.warning(f"⚠️ No files recognised")
            if file_roles:
                for role, label in file_roles.items():
                    st.caption(f"{'✅' if role in fb_display else '⬜'} {label}")
            else:
                for i, fname in enumerate(raw_bytes.keys()):
                    st.caption(f"✅ File {i+1} — {fname[:35]}")
    st.divider()
...
""")
    ...
    # Generic market — AUTOFR, IMMOUK, IMMODE, AUTOUK, EMPLOIFR, TRUCKSFR, AUTOBR, NPV
    SITES[:] = market_cfg["sites"]
    if not uploaded:
        st.markdown(f"## {market_cfg['emoji']} {market_cfg['label']} — Panel Quality Control")
        st.info("Upload your Excel files in the sidebar to begin.")
        st.stop()

    if market == "AUTOFR":
        checks,trends,fb,_mshare,tables = compute_everything_generic(file_hash, raw_bytes, "AUTOFR")
    else:
        checks,trends,fb,_mshare,tables = compute_everything_generic(file_hash, raw_bytes, market)
    ...
    n_warn = sum(1 for c in checks if c["sev"]=="warning")
    n_ok   = sum(1 for c in checks if c["ok"])
    MARKET_SITES_SET = set(market_cfg["sites"])
    _tr_sites_only = [r for r in trends if r["site"] in MARKET_SITES_SET]
    n_alert = len({f"{r['site']}_{r['sheet']}" for r in _tr_sites_only if r["status"]=="alert"})
    _warn_rows_ov = [r for r in _tr_sites_only if r["status"]=="warn"]
    n_surge = len({f"{r['site']}_{r['sheet']}" for r in _warn_rows_ov if _flag_is_surge(r.get("flags"))})
    n_wtr   = len({f"{r['site']}_{r['sheet']}" for r in _warn_rows_ov if not _flag_is_surge(r.get("flags"))})
    ...
    if "file1" in fb:
        try:
            _fb_val = fb["file1"]
            _fb_bytes = _fb_val[1] if isinstance(_fb_val, tuple) else _fb_val
            wb1 = load_wb_any(_fb_bytes)
            _sheets = wb1.sheetnames
            # Find sheet — try named sheets first, then scan for datetime columns
            ws_tot = (ws_get(wb1,"Total") or ws_get(wb1,"1.1 Total Ads") or
                      ws_get(wb1,"1.1 Total") or ws_get(wb1,"Panel") or
                      ws_get(wb1,"Websites"))
            if ws_tot is None:
                import datetime as _dt
                for _sn in _sheets:
                    if _sn.lower() in ("intro","feuil1","sheet1","readme","cover","3. shared","2. geo"): continue
                    _ws = wb1[_sn]
                    for _r in range(1, min(_ws.max_row+1, 15)):
                        for _c in range(1, min(_ws.max_column+1, 40)):
                            _v = _ws.cell(_r, _c).value
                            if isinstance(_v, _dt.datetime):
                                ws_tot = _ws; break
                            if isinstance(_v, str) and any(x in _v.lower() for x in ["-26","-25","-24","-23","jan","feb","mar","apr","may","jun","jul","aug","sep","oct","nov","dec"]):
                                ws_tot = _ws; break
                        if ws_tot: break
                    if ws_tot: break
            if ws_tot is None:
            ...
            if ws_tot:
                # Try standard read_series first (IMMOFR, IMMOLU style)
                d_tot = read_series(ws_tot, section=0)
                if not d_tot.get("_lm") or len([k for k in d_tot if not k.startswith("_")]) < 2:
                    # Try generic reader (AUTOLU paired-col style)
                    d_tot = read_series_generic(ws_tot)
                if not d_tot.get("_lm") or len([k for k in d_tot if not k.startswith("_")]) < 2:
                    # Try long format reader (IMMODE, IMMOUK style)
                    d_tot = read_long_format(ws_tot)
                lm_ref = d_tot.get("_lm","—"); pm_ref = d_tot.get("_pm","—")

                # If no months found, try scanning all rows for month pattern
                if lm_ref == "—":
                    import datetime as _dt
                    _mc = []
                    for _r in range(1, min(ws_tot.max_row+1, 15)):
                        for _c in range(1, min(ws_tot.max_column+1, 40)):
                            _v = ws_tot.cell(_r, _c).value
                            if isinstance(_v, _dt.datetime):
                                _mc.append((_c, _v.strftime("%b-%y")))
                            elif isinstance(_v, str) and any(x in _v.lower() for x in ["-26","-25","-24","-23"]):
                                _mc.append((_c, _v.strip()))
                        if len(_mc) >= 2: break
                    if len(_mc) >= 2:
                        lm_ref = _mc[-1][1]; pm_ref = _mc[-2][1]

                total = sv(d_tot,"Somme Panel") or sv(d_tot,"Total Panel") or sv(d_tot,"Total")
                dedup_label = next((k for k in d_tot if isinstance(k,str) and
                                    ("dédoublon" in k.lower() or "dedup" in k.lower())), None)
                dedup = sv(d_tot, dedup_label) if dedup_label else None
                _sites_from_file = [k for k in d_tot if isinstance(k,str) and not k.startswith("_")
                                    and k not in ("Total","Total Panel","Somme Panel")]
                # Update SITES for this market if empty
                if not market_cfg["sites"] and _sites_from_file:
                    market_cfg["sites"] = _sites_from_file[:20]
                    SITES[:] = market_cfg["sites"]
                n_active = sum(1 for s in market_cfg["sites"] if sv(d_tot,s) and sv(d_tot,s)>0)

                if lm_ref == "—":
                    _file1_error = f"No month columns in sheet '{ws_tot.title}'. Sheets: {_sheets}"
            wb1.close()
        except Exception as _e:
            _file1_error = str(_e)
    elif fb:
        _file1_error = f"file1 not recognised. fb keys: {list(fb.keys())}"
    ...
    with col_h:
        ...
        st.caption(f"Reference month: **{lm_ref}** · vs {pm_ref} · {len(fb)} files · v{APP_VERSION}")
    with col_f:
        chosen=st.multiselect("Filter by site", market_cfg["sites"], default=[],
                              placeholder="All sites", label_visibility="visible", key="gn_site_filter")
    site_filter=chosen if chosen else None
    ...
    with af_tab1:
        if _file1_error:
            st.warning(f"⚠️ Could not read file1: `{_file1_error}`. Check that the file format is correct (.xlsx) or xlrd is installed for .xls files.")
        if n_err>0:
        ...
        c3.metric("Active sites", f"{n_active} / {len(market_cfg['sites'])}")
...
""")
    ...
    with af_tab2:
        ...
        tr=[r for r in trends if not site_filter or r["site"] in site_filter]
        MARKET_SITES_SET = set(market_cfg["sites"]) if market_cfg["sites"] else set(r["site"] for r in tr)
        tr_sites=[r for r in tr if r["site"] in MARKET_SITES_SET] or tr
        ...
        with af_sub_m:
            ...
            # AUTOFR "Evolution" sheet specifically: it places "Particuliers" and
            # "Professionnels" side by side after the same header row (see
            # _month_runs), so a single site there shows up as 2 short (2-month)
            # segments. Fetch both and show them TOGETHER in one grouped chart
            # instead of one confusing single-segment card. Scoped to this exact
            # sheet only — no other AUTOFR/IMMOFR sheet is affected.
            _seg_cache = {}
            for r in flagged_m:
                if r["sheet"] != "Evolution":
                    continue
                n_real_r = sum(1 for v in (r.get("vals") or []) if v is not None)
                if n_real_r > 2:
                    continue
                ck = (r["file"], r["sheet"])
                if ck not in _seg_cache:
                    _seg_cache[ck] = {}
                if r["site"] not in _seg_cache[ck]:
                    _seg_cache[ck][r["site"]] = sheet_all_segments_for_site(
                        raw_bytes, r["file"], r["sheet"], r["site"])
                segs = _seg_cache[ck][r["site"]]
                if len(segs) >= 2:
                    r["segments"] = segs
    ...
    with af_tab_dq:
        ...
        if site_filter:
            checks_show=[c for c in checks if any(s.lower() in c["name"].lower() for s in site_filter)
                         or not any(s.lower() in c["name"].lower() for s in market_cfg["sites"])]
        tables_show=tables
        if site_filter:
