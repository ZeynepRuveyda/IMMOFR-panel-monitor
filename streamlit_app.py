...
MARKET_CONFIG = {
    ...
    "IMMOLU":   {"key":"IMMOLU",   "label":"IMMO LU",   "emoji":"🏠", "sites":["Immotop.lu","Athome.lu","Logic-immo.lu","Era.lu","Wortimmo.lu"],                       "file_roles":{"file1":"1 — Panel evolution","file2":"2 — Quality metrics","file3":"3 — Professionals","file4":"4 — Geographic stats","file5":"5 — Focus Grand-Duché"}, "group_info":{"1":("Panel evolution","Volumes"),"2":("Quality metrics","Freshness"),"3":("Professionals","Pro counts"),"4":("Geographic stats","Regional breakdown"),"5":("Focus Grand-Duché","Luxembourg detailed")}, "classify_fn":"generic","run_checks_fn":"generic","has_special_check":False,"has_dept_region":False},
    "AUTOLU":   {"key":"AUTOLU",   "label":"AUTO LU",   "emoji":"🚗", "sites":["Autoscout24.lu","Luxauto.lu","Motortrend.lu","Mobile.lu","Cardosso.lu"],               "file_roles":{"file1":"1 — Panel evolution","file2":"2 — Quality metrics"},                                          "group_info":{"1":("Panel evolution","Volumes"),"2":("Quality metrics","Freshness")},                                    "classify_fn":"generic","run_checks_fn":"generic","has_special_check":False,"has_dept_region":False},
}
...
def classify_generic(raw, file_roles):
    ...
    for fname, data in raw.items():
        ...
        for role, label in file_roles.items():
            ...
            if "evolution" in nl or "panel" in nl:
                out.setdefault("file1", data)
            elif "qualit" in nl or "perform" in nl:
...
""")
    ...
    n_warn = sum(1 for c in checks if c["sev"]=="warning")
    n_ok   = sum(1 for c in checks if c["ok"])
    AUTOFR_SITES_SET = set(market_cfg["sites"])
