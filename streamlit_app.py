...
<tbody>{rows_html}</tbody>
    ...
    with subtab_m:
        ...
        with st.expander("📋 Detailed table — click to expand for in-depth review", expanded=False):
            ...
            # flags zaten mom_flags olarak set edildi
            tr_m_filtered = tr_m_show
    ...
    with subtab_y:
        ...
        st.caption(f"**Y-1 data for {n_y1} series** — 🔴 {n_crit_y} critical · 🟡 {n_warn_y} warnings"
                   f"  \n*Includes: Crash vs 12m peak (<60%), M/Y-1 change >±30%, 3-month downtrend >-5%*")
    ...
    with subtab_d:
        ...
        st.caption(f"**Department-level series (M/M-1)** — 🔴 {n_crit_d} critical · 🟡 {n_warn_d} warnings"
                   f"  \n*Critical = drop ≥20% · Warning = drop 10-20%, surge ≥30%, crash vs peak, downtrend*")
