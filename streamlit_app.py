...
tab1,tab2,tab3,tab4,tab_ms=st.tabs(["📋  Overview","📈  Monthly/Yearly Trends","🔍  Data integrity","📊  Table analysis","🎯  Special check for LBC"])
        ...
        if not attention_df.empty:
            ...
            st.dataframe(
                ...
                column_config={
                    ...
                    "Reason":       st.column_config.TextColumn(width=180),
                },
            )
        else:
            st.divider()
        ...
        if not df_site.empty:
            st.dataframe(
                ...
                column_config={
                    ...
                    "Reason":            st.column_config.TextColumn(width=160),
                },
            )
        else:
...
with tab2:
    ...
    # Geographic files — département seçici gösterilecek dosyalar
    GEO_FILES = {"3.2 — Geographic pros", "4.1 — Geographic stats",
                 "4.2 — Exclusivity & sharing", "5.2 — Grand Ouest",
                 "5 — Focus IDF", "6 — New announcements IDF"}

    # tr'deki her row'un hangi dosyadan geldiğini bil
    # fname = classify key → FILE_ROLES label
    def get_file_label(fname):
        for role, label in FILE_ROLES.items():
            if label.lower() in fname.lower() or role.lower() in fname.lower():
                return label
        # fallback: dosya adından tahmin
        for label in FILE_ROLES.values():
    ...
    # ── Sheet (file) filter — önce seç ──
    all_sheets_tr = sorted(set(r["sheet"] for r in tr))
    sheet_sel = st.multiselect("Filter by sheet", all_sheets_tr,
                               placeholder="All sheets", label_visibility="visible", key="tr_sheet")
    tr_sheet_filtered = [r for r in tr if r["sheet"] in sheet_sel] if sheet_sel else tr

    # Geographic mi kontrol et — seçilen sheet'ler geo dosyasına ait mi?
    # Sheet adından dosyayı anlamak zor, fname'e bak
    # tr row'larında fname var mı? build_trends'e bakınca fname=file adı
    # Basit yaklaşım: sheet adında "Région", "Dépt", "département" varsa geo
    def is_geo_sheet(sheet_name):
        sl = sheet_name.lower()
        return any(x in sl for x in ["région", "dept", "département", "géograph",
                                      "annonceur", "grand ouest", "idf", "alpes",
                                      "nouvelle", "focus", "exclusiv"])


    all_sites_raw = sorted(set(r["site"] for r in tr_sheet_filtered if r["status"]!="inactive"))

    # Geo modda: sadece numara+isim formatını tut (duplicate isimleri kaldır)
    panel_sites_in_list = PANEL_SITES_SET & set(all_sites_raw)
    is_geo_mode = len(panel_sites_in_list) < max(1, len(all_sites_raw) * 0.3)

    if is_geo_mode:
        # Numaralı olanlar kalır, sadece isim olanlar kalkar
        all_sites_tr = [s for s in all_sites_raw if _has_dept_number(s)]
        if not all_sites_tr:  # fallback: hepsi kalsın
            all_sites_tr = all_sites_raw
    else:
        all_sites_tr = all_sites_raw

    site_label = "Department" if is_geo_mode else "Website"
    sel_site_tr = st.selectbox(site_label, ["All"] + all_sites_tr, key="tr_site")

    tr_filtered = [r for r in tr_sheet_filtered if sel_site_tr == "All" or r["site"] == sel_site_tr]

    # Geo modda ve dept seçildiyse → sub-area seçici göster
    if is_geo_mode and sel_site_tr != "All":
        depts_available = sorted(set(r.get("section","") for r in tr_filtered if r.get("section","")))
        if depts_available:
            sel_dept = st.selectbox("Sub-area", ["All"] + depts_available, key="tr_dept")
            if sel_dept != "All":
                tr_filtered = [r for r in tr_filtered if r.get("section","") == sel_dept]

    st.divider()

    # ── Sub-tabs: Monthly / Yearly / Département ──
    subtab_m, subtab_y, subtab_d = st.tabs([
        "📅 Monthly (M/M-1)",
        "📆 Yearly (M/Y-1)",
        "🗺️ By département",
    ])
    ...
    def _trend_table(rows, cols_extra=None):
        """Ortak tablo renderer."""
        def trend_status_icon(s):
            return "🔴" if s=="alert" else "🟡" if s=="warn" else "⚪" if s=="inactive" else "✅"
        def trend_label(r):
            sec=r.get("section",""); sheet=r["sheet"]
            return sec[:30] if sec and sec not in (sheet,"") else sheet[:30]
        ...
        base = [{
            ...
            **({"M/Y-1": f"{r.get('evol_y1'):+.1f}%" if r.get("evol_y1") is not None else "—"} if cols_extra=="y1" else {}),
            "Mkt share": f"{r['prm']:.1f}%" if r["prm"] is not None else "—",
            "Notes": " · ".join(r.get("flags",[]) or []) or ("Not reporting" if r["status"]=="inactive" else "—"),
        } for r in sorted(rows, key=lambda x: {"alert":0,"warn":1,"ok":2,"inactive":3}[x["status"]])]
        return pd.DataFrame(base)

    # ─── MONTHLY (M/M-1) — site-level only ───
    with subtab_m:
        tr_site = [r for r in tr_filtered
                   if not _has_dept_number(r["site"]) and
                   any(s.lower() in r["site"].lower() for s in
                       ["AvendreAlouer","Bien","Figaro","Green","Leboncoin","Logic",
                        "Meilleurs","Ouest","PAP","Paru","SeLoger","Immo","Super","Avendre"])
                   or r["site"] in PANEL_SITES_SET]

        # Fallback: geo mode değilse tr_filtered'ın tamamı
        if not is_geo_mode:
            tr_site = tr_filtered
        ...
        seen_m = set()
        for r in tr_site:
            if r["status"] in ("alert","warn") and r.get("flags"):
        ...
        df_m = _trend_table(tr_site)
        if not df_m.empty:
            st.dataframe(df_m, use_container_width=True, hide_index=True,
                         height=min(500, 42+35*len(df_m)))
            st.download_button("⬇ Download", df_m.to_csv(index=False).encode("utf-8-sig"),
                               f"monthly_{lm_ref}.csv", "text/csv")

    # ─── YEARLY (M/Y-1) — site-level only ───
    with subtab_y:
        flagged_y = []
        seen_y = set()
        for r in (tr_filtered if not is_geo_mode else tr_site if tr_site else tr_filtered):
            y1_flags = [f for f in (r.get("flags") or []) if "Y-1" in f or "year" in f.lower()]
            if r.get("evol_y1") is not None and (y1_flags or r["status"] in ("alert","warn")):
                k = f"{r['site']}_{r['sheet']}"
                if k not in seen_y:
                    seen_y.add(k)
        ...
        df_y = _trend_table(tr_filtered if not is_geo_mode else tr_site if tr_site else tr_filtered, cols_extra="y1")
        df_y = df_y[df_y["M/Y-1"] != "—"] if "M/Y-1" in df_y.columns else df_y
        if not df_y.empty:
            st.dataframe(df_y, use_container_width=True, hide_index=True,
                         height=min(500, 42+35*len(df_y)))
            st.download_button("⬇ Download", df_y.to_csv(index=False).encode("utf-8-sig"),
                               f"yearly_{lm_ref}.csv", "text/csv")
        else:
            st.info("No Y-1 data available.")

    # ─── DÉPARTEMENT — geo rows, M/M-1 only ───
    with subtab_d:
        tr_dept = [r for r in tr_filtered if _has_dept_number(r["site"])]

        if not tr_dept and is_geo_mode:
            tr_dept = tr_filtered  # fallback
        ...
        for r in tr_dept:
            if r["status"] in ("alert","warn") and r.get("flags"):
                ...
                if mom_flags:
                    ...
                    if k not in seen_d:
                        seen_d.add(k)
                        flagged_d.append({**r, "flags": mom_flags})
        ...
        st.divider()
        st.markdown("#### All département series — M/M-1")
        df_d = _trend_table(tr_dept)
        if not df_d.empty:
            st.dataframe(df_d, use_container_width=True, hide_index=True,
                         height=min(500, 42+35*len(df_d)))
            st.download_button("⬇ Download", df_d.to_csv(index=False).encode("utf-8-sig"),
                               f"dept_{lm_ref}.csv", "text/csv")
        else:
            st.info("No département data found. Select a geographic sheet above.")
