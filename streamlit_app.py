...
""")
    ...
    with af_tab2:
        ...
        tr=[r for r in trends if not site_filter or r["site"] in site_filter]
        tr_sites=[r for r in tr if r["site"] in AUTOFR_SITES_SET]

        with af_sub_m:
            flagged_m=[r for r in tr_sites if r["status"] in ("alert","warn")]
            n_crit=sum(1 for r in flagged_m if r["status"]=="alert")
            n_surge_m=sum(1 for r in flagged_m if r["status"]=="warn" and _flag_is_surge(r.get("flags")))
            n_warn_m=sum(1 for r in flagged_m if r["status"]=="warn" and not _flag_is_surge(r.get("flags")))
            st.caption(f"**Website-level series** — 🔴 {n_crit} critical · 🟠 {n_surge_m} surge · 🟡 {n_warn_m} warnings"
                       f"  \n*Critical = M/M-1 drop ≥ 20% · Surge = ≥ 30% increase · Warning = decline 10-20%*")
            if flagged_m:
                cols3=st.columns(3)
                for i,row in enumerate(sorted(flagged_m,key=lambda r:({"alert":0,"warn":1}.get(r["status"],2)))[:18]):
                    clr=status_color(row["status"],row.get("flags"),kind="line")
                    with cols3[i%3]:
                        vals=[v if v is not None else None for v in row["vals"]]
                        x_vals=row["months"][:len(vals)]
                        fig=go.Figure(go.Scatter(x=x_vals,y=vals,mode="lines+markers",
                            line=dict(color=clr,width=2.5),
                            marker=dict(size=[7 if k==len(vals)-1 else 0 for k in range(len(vals))]),
                            connectgaps=False,hovertemplate="%{x}: %{y:,.0f}<extra></extra>"))
                        fig.update_layout(height=110,margin=dict(l=0,r=0,t=0,b=0),
                            paper_bgcolor="rgba(0,0,0,0)",plot_bgcolor="rgba(0,0,0,0)",
                            xaxis=dict(showgrid=False,tickfont=dict(size=8),nticks=4),
                            yaxis=dict(showgrid=True,gridcolor="rgba(180,180,180,.3)",tickfont=dict(size=8),tickformat=".2s"),
                            showlegend=False)
                        evol_s=f"{row['evol']:+.1f}%" if row.get("evol") is not None else ""
                        lines_lbl=[]
                        if row.get("file"): lines_lbl.append(f"📁 {row['file']}")
                        if row.get("sheet"): lines_lbl.append(f"📄 {row['sheet']}")
                        if row.get("section") and row["section"]!=row.get("sheet"): lines_lbl.append(f"📊 {row['section']}")
                        full_label="  \n".join(f"*{l}*" for l in lines_lbl)
                        st.markdown(f"**{row.get('site','')}** {evol_s}  \n{full_label}")
                        st.plotly_chart(fig,use_container_width=True,config={"displayModeBar":False},key=f"af_m_{i}")
                with st.expander("📋 Detailed table",expanded=False):
                    df_rows=[{
                        "Status":status_icon(r["status"],r.get("flags")),
                        "Website":r["site"],"File":r.get("file",""),"Sheet":r.get("sheet",""),
                        f"M ({r['lm']})":_fmt_full(r["lv"]),f"M-1 ({r['pm']})":_fmt_full(r["pv"]),
                        "M/M-1":f"{r['evol']:+.1f}%" if r["evol"] is not None else "—",
                        "ℹ️":" · ".join(r.get("flags") or []),
                    } for r in flagged_m]
                    _render_scrollable_df(pd.DataFrame(df_rows))
            else:
                st.info("No anomalies detected at website level for this month.")

        with af_sub_y:
            tr_y=[r for r in tr_sites if r.get("evol_y1") is not None]
            flagged_y=[r for r in tr_y if abs(r["evol_y1"])>30]
            st.caption(f"**M/Y-1 series** — {len(flagged_y)} flagged out of {len(tr_y)} with a full year of history")
            if flagged_y:
                df_y=pd.DataFrame([{
                    "Status":"🔴" if r["evol_y1"]<=-30 else "🟠" if r["evol_y1"]>=30 else "🟡",
                    "Website":r["site"],"File":r.get("file",""),"Sheet":r.get("sheet",""),
                    f"M ({r['lm']})":_fmt_full(r["lv"]),
                    "M/Y-1":f"{r['evol_y1']:+.1f}%",
                } for r in flagged_y])
                _render_scrollable_df(df_y)
            else:
                st.info("No M/Y-1 anomalies detected (or not enough history yet).")
            ...
            if not tables_show:
                ...
                if not tables_f:
                    ...
                    if sel_t["sites"]:
                        df_data=pd.DataFrame([{
                            "":"🔴" if r["status"]=="alert" else ("🟠" if r.get("is_surge") else "🟡") if r["status"]=="warn" else "✅",
                            "Website":r["site"],f"M ({sel_t['lm']})":_fmt_full(r["last"]),
                            f"M-1 ({sel_t['pm']})":_fmt_full(r["prev"]),
                            "M/M-1":f"{r['evol']:+.1f}%" if r["evol"] is not None else "—",
                        } for r in sorted(sel_t["sites"],key=lambda x:({"alert":0,"warn":1,"ok":2}[x["status"]],-(x["last"] or 0)))])
                        def _af_cr(df):
                            def _row(row):
