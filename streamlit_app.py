...
MARKET_CONFIG = {
    ...
    "IMMOUK":   {"key":"IMMOUK",   "label":"IMMO UK",   "emoji":"🏠", "sites":[], "file_roles":{}, "group_info":{}, "classify_fn":"generic","run_checks_fn":"generic","has_special_check":False,"has_dept_region":False},
    "IMMODE":   {"key":"IMMODE",   "label":"IMMO DE",   "emoji":"🏠", "sites":[], "file_roles":{}, "group_info":{}, "classify_fn":"generic","run_checks_fn":"generic","has_special_check":False,"has_dept_region":False},
...
""")
    ...
    if "file1" in fb:
        try:
            ...
            if ws_tot:
                # Try standard read_series first (works for IMMOFR, IMMOLU style)
                d_tot = read_series(ws_tot, section=0)
                if not d_tot.get("_lm") or len([k for k in d_tot if not k.startswith("_")]) < 2:
                    # Fallback to generic reader (works for AUTOLU paired-col style)
