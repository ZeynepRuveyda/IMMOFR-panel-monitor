...
MARKET_CONFIG = {
    ...
    "IMMOUK":   {"key":"IMMOUK",   "label":"IMMO UK",   "emoji":"🏠", "sites":["Rightmove","Zoopla","OnTheMarket","Primelocation","Boomin"],   "file_roles":{"file1":"1 — Panel evolution","file2":"2 — Quality metrics","file3":"3 — Professionals"},   "group_info":{"1":("Panel evolution","Volumes"),"2":("Quality metrics","Freshness"),"3":("Professionals","Pro counts")}, "classify_fn":"generic","run_checks_fn":"generic","has_special_check":False,"has_dept_region":False},
    "IMMODE":   {"key":"IMMODE",   "label":"IMMO DE",   "emoji":"🏠", "sites":["ImmoScout24","Immowelt","Immonet","eBay Kleinanzeigen","Engel & Völkers"],           "file_roles":{"file1":"1 — Panel evolution","file2":"2 — Quality metrics"},                                          "group_info":{"1":("Panel evolution","Volumes"),"2":("Quality metrics","Freshness")},                                    "classify_fn":"generic","run_checks_fn":"generic","has_special_check":False,"has_dept_region":False},
    "AUTOUK":   {"key":"AUTOUK",   "label":"AUTO UK",   "emoji":"🚗", "sites":["AutoTrader","Motors","CarGurus","Cazoo","Arnold Clark"],                              "file_roles":{"file1":"1 — Panel evolution","file2":"2 — Performance"},                                               "group_info":{"1":("Panel evolution","Volumes"),"2":("Performance","Freshness")},                                        "classify_fn":"generic","run_checks_fn":"generic","has_special_check":False,"has_dept_region":False},
    "EMPLOIFR": {"key":"EMPLOIFR", "label":"EMPLOI FR", "emoji":"📰", "sites":["Indeed","LinkedIn","Pôle Emploi","Monster","Cadremploi"],                             "file_roles":{"file1":"1 — Panel evolution","file2":"2 — Quality metrics"},                                          "group_info":{"1":("Panel evolution","Volumes"),"2":("Quality metrics","Freshness")},                                    "classify_fn":"generic","run_checks_fn":"generic","has_special_check":False,"has_dept_region":False},
    "TRUCKSFR": {"key":"TRUCKSFR", "label":"TRUCKS FR", "emoji":"🚛", "sites":["Leboncoin","ParuVendu","Truck1","Truckscout24","Mascus"],                             "file_roles":{"file1":"1 — Panel evolution","file2":"2 — Quality metrics"},                                          "group_info":{"1":("Panel evolution","Volumes"),"2":("Quality metrics","Freshness")},                                    "classify_fn":"generic","run_checks_fn":"generic","has_special_check":False,"has_dept_region":False},
    "AUTOBR":   {"key":"AUTOBR",   "label":"AUTO BR",   "emoji":"🚗", "sites":["OLX","Webmotors","iCarros","AutoLine","Mercado Livre"],                               "file_roles":{"file1":"1 — Panel evolution","file2":"2 — Quality metrics"},                                          "group_info":{"1":("Panel evolution","Volumes"),"2":("Quality metrics","Freshness")},                                    "classify_fn":"generic","run_checks_fn":"generic","has_special_check":False,"has_dept_region":False},
    "NPV":      {"key":"NPV",      "label":"NPV UK",    "emoji":"🏠", "sites":["Rightmove","Zoopla","OnTheMarket","Primelocation"],                                  "file_roles":{"file1":"1 — Panel evolution","file2":"2 — Quality metrics","file3":"3 — New builds"},                    "group_info":{"1":("Panel evolution","Volumes"),"2":("Quality metrics","Freshness"),"3":("New builds","NPV checks")},    "classify_fn":"generic","run_checks_fn":"generic","has_special_check":False,"has_dept_region":False},
...
def classify_generic(raw, file_roles):
    """Generic classify — matches files to roles by number prefix or keyword."""
    out = {}
    # First pass: match by number in filename
    for fname, data in raw.items():
        nl = norm(fname)
        for role, label in file_roles.items():
            if role in out: continue
            num = role.replace("file","").replace("_",".")
            num_simple = role.replace("file","")  # "1", "2", "3_1"→"3_1"
            if any(p in nl for p in [f"_{num}_", f"_{num}.", f"_{num_simple}_",
                                      f"_{num_simple}.", f"-{num}-", f"{num_simple}_"]):
                out[role] = data; break
    # Second pass: keyword fallback for unmatched files
    for fname, data in raw.items():
        nl = norm(fname)
        if "file1" not in out and any(k in nl for k in ["evolution","panel","website","total"]):
            out["file1"] = data
        elif "file2" not in out and any(k in nl for k in ["qualit","perform","offer"]):
            out["file2"] = data
        elif "file3" not in out and any(k in nl for k in ["pros","agent","professional"]):
            out["file3"] = data
    return out
...
with st.sidebar:
    ...
    if uploaded:
        ...
        if market_cfg["classify_fn"] == "autofr":
            ...
            fb_display = classify_generic(raw_bytes, file_roles)
            n_ok_files = len(fb_display)
        else:
    ...
    if "file1" in fb:
        try:
            ...
            ws_tot = (ws_get(wb1,"Total") or ws_get(wb1,"1.1 Total Ads") or
                      ...
                      ws_get(wb1,"Websites") or wb1.active)
            if ws_tot:
                d_tot=read_series(ws_tot,section=0)
                lm_ref=d_tot.get("_lm","—"); pm_ref=d_tot.get("_pm","—")
                total=sv(d_tot,"Somme Panel") or sv(d_tot,"Total")
