...
with tab2:
    ...
    def _trend_table_html(rows, title="All series — M/M-1", cols_extra=None):
        ...
        for r in sorted_rows:
            ...
            lv = _html.escape(fmt(r["lv"])); pv = _html.escape(fmt(r["pv"]))
...
.nt::after {{
  ...
  font-size:12px;white-space:normal;min-width:260px;max-width:420px;
  line-height:1.5;z-index:999;pointer-events:none;opacity:0;transition:opacity .15s;
...
<tbody>{rows_html}</tbody>
    ...
    with subtab_m:
        ...
        with st.expander("📋 Detailed table — click to expand for in-depth review", expanded=False):
            ...
            html_m = _trend_table_html(tr_m_show)
        ...
        with st.expander("📋 Detailed table — click to expand for in-depth review", expanded=False):
            ...
            for r in rows_to_show_d:
                ...
                df_d_rows.append({
                    ...
                    f"M-1 ({r['pm']})": fmt(r["pv"]),
                    "M/M-1": f"{r['evol']:+.1f}%" if r["evol"] is not None else "—",
                    "Notes": " · ".join(mom_notes) if mom_notes else "—",
                })
