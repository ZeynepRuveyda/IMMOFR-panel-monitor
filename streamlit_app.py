...
def read_long_format(ws):
    ...
    if "website" in h_lower and "type" in h_lower:
        ...
        # Value col = last numeric col header
        ...
        TARGET_TYPES = {"listings","total listings","ads"}
        out = {}; lm = None
        for r in range(2, ws.max_row+1):
            site = ws.cell(r, site_col).value
            ...
            mo   = ws.cell(r, mo_col).value
            if not site or not isinstance(val, (int,float)): continue
            if site in ("Market","market",None): continue
            if not any(t in typ for t in TARGET_TYPES): continue
