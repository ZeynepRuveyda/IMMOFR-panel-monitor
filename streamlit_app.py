...
def analyse_all_tables(raw_bytes_dict):
    ...
    for role,data in fb.items():
        ...
        for sn in wb.sheetnames:
            if sn=="Intro": continue
            ws=wb[sn]
            sections=[read_series(ws,section=i) for i in range(len([r for r in range(1,ws.max_row+1) if ws.cell(r,2).value in ("Site","Département","Région")]))]
...
with tab4:
    ...
    # ── Table header ──
    ttype_lbl = "📊 Volume (listings)" if sel_t["table_type"]=="volume" else "📐 Rate table (ratios 0–1)"
