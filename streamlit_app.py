...
def classify_lu(raw, file_roles=None):
    """Classify IMMOLU/AUTOLU files by name pattern (1_Websites, 2_Performance, etc.)"""
    out = {}
    for fname, data in raw.items():
        nl = fname.lower()
        if "1_website" in nl or "1_web" in nl:
...
""")
    ...
    if "file1" in fb:
        try:
            ...
            ws_tot=ws_get(wb1,"Total") or ws_get(wb1,"1.1 Total") or ws_get(wb1,"Panel")
            if ws_tot:
