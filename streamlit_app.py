...
with tab2:
    ...
    # ── Sub-tabs: Monthly / Yearly / Département / Région ──
    ...
    with subtab_m:
        tr_site = [r for r in tr_filtered
                   if r["site"] in PANEL_SITES_SET]
        if not is_geo_mode:
            tr_site = tr_filtered

        n_crit_m = sum(1 for r in tr_site if r["status"]=="alert")
        n_warn_m = sum(1 for r in tr_site if r["status"]=="warn")
        st.caption(f"**Website-level series only** — 🔴 {n_crit_m} critical · 🟡 {n_warn_m} warnings")
    ...
    with subtab_y:
        tr_y_base = tr_filtered if not is_geo_mode else tr_site if tr_site else tr_filtered
        n_y1 = sum(1 for r in tr_y_base if r.get("evol_y1") is not None)
        n_crit_y = sum(1 for r in tr_y_base if r["status"]=="alert" and r.get("evol_y1") is not None)
        n_warn_y = sum(1 for r in tr_y_base if r["status"]=="warn" and r.get("evol_y1") is not None)
        st.caption(f"**Y-1 data available for {n_y1} series** — 🔴 {n_crit_y} critical · 🟡 {n_warn_y} warnings")
        seen_y = set()
    ...
    with subtab_d:
        ...
        n_crit_d = sum(1 for r in tr_dept if r["status"]=="alert")
        n_warn_d = sum(1 for r in tr_dept if r["status"]=="warn")
        st.caption(f"**Department-level series only (M/M-1)** — 🔴 {n_crit_d} critical · 🟡 {n_warn_d} warnings")  # fallback
    ...
    with subtab_r:
        tr_region = [r for r in tr_filtered if _is_region(r["site"])]

        n_crit_r = sum(1 for r in tr_region if r["status"]=="alert")
        n_warn_r = sum(1 for r in tr_region if r["status"]=="warn")
        st.caption(f"**Région-level series only (M/M-1)** — 🔴 {n_crit_r} critical · 🟡 {n_warn_r} warnings")
