...
""")
    ...
    if "file1" in fb:
        try:
            ...
            # Try named sheets first, then scan all sheets for one with time series
            ws_tot = (ws_get(wb1,"Total") or ws_get(wb1,"1.1 Total Ads") or
                      ws_get(wb1,"1.1 Total") or ws_get(wb1,"Panel") or
                      ws_get(wb1,"Websites"))
            if ws_tot is None:
                # Scan all sheets — pick first one with month columns
                import datetime as _dt
                for _sn in _sheets:
                    if _sn.lower() == "intro": continue
                    _ws = wb1[_sn]
                    for _r in range(1, min(_ws.max_row+1, 10)):
                        for _c in range(1, min(_ws.max_column+1, 30)):
