...
# ═══════════════════════════════════════════════
# TABLE ANALYSIS — per-sheet, per-section deep dive
# ═══════════════════════════════════════════════

def _detect_table_type(d):
    """
    Bir section'ın 'taux' (oran 0-1) mu yoksa 'volume' (sayı) tablosu mu olduğunu tespit et.
    """
    vals = []
    for k, v in d.items():
        if not isinstance(k, str) or k.startswith("_") or k in SKIP: continue
        last = v.get("last")
        if last is not None and isinstance(last, (int, float)) and last > 0:
            vals.append(last)
    if not vals: return "volume"
    ratio_vals = sum(1 for v in vals if 0 < v <= 1.5)
    return "taux" if ratio_vals / len(vals) >= 0.75 else "volume"

def _table_qc_issues(d, table_type, label):
    """
    Bir section için QC sorunlarını tespit et.
    Taux tablolarında farklı, volume tablolarında farklı kurallar uygula.
    """
    issues = []
    lm = d.get("_lm", "?")
    pm = d.get("_pm", "?")
    ...
    for k, v in d.items():
        if isinstance(k, str) and ("dédupliqué" in k.lower() or "dedup" in k.lower()) and not k.startswith("_"):
            dedup = v.get("last"); dedup_label = k; break
    ...
    if table_type == "taux":
        # Taux tablosunda: değerler 0-1 arası oran (yüzde/100)
        # Kontrol 1: önceki ay 0 iken bu ay değer var (veya tersi)
        # Kontrol 2: M vs M-1 arasında büyük mutlak değişim (pp cinsinden)
        # Kontrol 3: taux > 1 (>100%) — formül hatası
        for k, v in d.items():
            if not isinstance(k, str) or k.startswith("_") or k in SKIP: continue
            last = v.get("last"); prev = v.get("prev")
            if last is None: continue

            # Taux > 100% — formül hatası
            if isinstance(last, (int, float)) and last > 1.05:
                issues.append({
                    "type": "TAUX>100%",
                    "severity": "error",
                    "site": k,
                    "message": f"Taux = {last*100:.1f}% > 100% — impossible, calculation error in source file",
                    "values": f"{k}: {last*100:.1f}% en {lm}",
                })
                continue

            if not isinstance(last, (int, float)): continue

            # Prev 0, last > 0 (sudden appearance)
            if prev is not None and isinstance(prev, (int,float)):
                if prev == 0 and last > 0.005:
                    issues.append({
                        "type": "TAUX_ZERO_TO_VALUE",
                        "severity": "warning",
                        "site": k,
                        "message": f"Rate was 0% en {pm}, now {last*100:.2f}% en {lm} — sudden appearance",
                        "values": f"{k}: {pm}=0% → {lm}={last*100:.2f}%",
                    })
                # Last 0, prev > 0 (disparition soudaine)
                elif last == 0 and prev > 0.005:
                    issues.append({
                        "type": "TAUX_VALUE_TO_ZERO",
                        "severity": "error",
                        "site": k,
                        "message": f"Rate was {prev*100:.2f}% en {pm}, dropped to 0% en {lm} — data missing?",
                        "values": f"{k}: {pm}={prev*100:.2f}% → {lm}=0%",
                    })
                # Grand saut en pp (variation absolue)
                elif prev > 0.001 and last > 0:
                    delta_pp = (last - prev) * 100  # in percentage points
                    delta_rel = (last / prev - 1) * 100  # en % relative
                    # Only flag if absolute change > 3pp AND relative > 50%
                    # AND at least one value is meaningful (>1%)
                    if abs(delta_pp) > 3 and abs(delta_rel) > 50 and max(last, prev) > 0.01:
                        sev = "error" if abs(delta_pp) > 8 or abs(delta_rel) > 100 else "warning"
                        issues.append({
                            "type": "TAUX_JUMP",
                            "severity": sev,
                            "site": k,
                            "message": f"Taux: {prev*100:.2f}% → {last*100:.2f}% (Δ {delta_pp:+.2f}pp, {delta_rel:+.0f}% relative)",
                            "values": f"{k}: {pm}={prev*100:.2f}% → {lm}={last*100:.2f}%",
                        })

    else:  # volume table
        # Kural: her ani değişim alarm — düşüş veya yükseliş fark etmez
        site_data = {k: v for k, v in d.items()
                     if isinstance(k, str) and not k.startswith("_") and k in SITES}
        ...
        for site, sd in site_data.items():
            ...
            if last is not None and prev and isinstance(prev, (int,float)) and prev > 100:
                evol = (last / prev - 1) * 100
                abs_change = abs(last - prev)

                # Ignore if both values are small (noisy low-volume data)
                # Require: previous month ≥ 100 AND absolute change ≥ 50
                if prev < 100 or abs_change < 50:
                    pass
                elif abs(evol) >= 30:
                    issues.append({
                        "type": "CHANGE_SEVERE",
                        "severity": "error",
                        "site": site,
                        "message": f"{evol:+.1f}% — {pm}: {_fmtn(prev)} → {lm}: {_fmtn(last)}",
                        "values": f"{site}: {pm}={_fmtn(prev)} → {lm}={_fmtn(last)}",
                    })
                elif abs(evol) >= 20:
                    issues.append({
                        "type": "CHANGE",
            ...
            # Market share > 100% — site vs Total (not dedup)
            if last and total and isinstance(total, (int,float)) and total > 100:
                ms = last / total * 100
                if ms > 100.5:
                    issues.append({
                        "type": "MS_OVER_100",
                        "severity": "error",
                        "site": site,
                        "message": f"Site listings ({_fmtn(last)}) > Total ({_fmtn(total)}) → {ms:.0f}% of total — impossible",
                        "values": f"{site}: {_fmtn(last)} / Total {_fmtn(total)} = {ms:.0f}%",
                    })

        # Dedup M vs M-1 — find dedup row and compare to previous month
        dedup_val = None
        dedup_prev_val = None
        for k, v in d.items():
            if isinstance(k, str) and ("dédupliqué marché" in k.lower() or "dedup marché" in k.lower()) and not k.startswith("_"):
                dedup_val = v.get("last")
                dedup_prev_val = v.get("prev")
                break
        ...
        if (dedup_val and dedup_prev_val and
                isinstance(dedup_val, (int,float)) and isinstance(dedup_prev_val, (int,float)) and
                dedup_prev_val > 100):
            dedup_evol = (dedup_val / dedup_prev_val - 1) * 100
            abs_change = abs(dedup_val - dedup_prev_val)
            if abs(dedup_evol) >= 30 and abs_change >= 50:
                issues.append({
                    "type": "DEDUP_CHANGE_SEVERE",
                    "severity": "error",
                    "site": "Total dédupliqué",
                    "message": f"Deduplicated total: {dedup_evol:+.1f}% — {pm}: {_fmtn(dedup_prev_val)} → {lm}: {_fmtn(dedup_val)}",
                    "values": f"Dedup: {pm}={_fmtn(dedup_prev_val)} → {lm}={_fmtn(dedup_val)}",
                })
            elif abs(dedup_evol) >= 20 and abs_change >= 50:
                issues.append({
                    "type": "DEDUP_CHANGE",
                    "severity": "warning",
                    "site": "Total dédupliqué",
        ...
        # Somme sites ≠ total (si total présent et significatif)
        if total and isinstance(total, (int,float)) and total > 500:
            site_sum = sum(sd["last"] for sd in site_data.values()
                          if sd.get("last") and isinstance(sd["last"], (int,float)))
            if site_sum > 0:
                gap_pct = abs(site_sum - total) / total * 100
                if gap_pct > 20:
                    issues.append({
                        "type": "SUM_MISMATCH",
                        "severity": "warning",
                        "site": "TOTAL",
                        "message": (f"Somme des {len(site_data)} sites ({_fmtn(site_sum)}) ≠ Total ({_fmtn(total)}) "
                                    f"— écart {gap_pct:.0f}%. Normal si le Total inclut des sites hors panel "
                                    f"(Immobilier Notaire, Immonot...)."),
                        "values": f"Σ sites={_fmtn(site_sum)} | Total={_fmtn(total)} | écart={_fmtn(abs(site_sum-total))}",
                    })
        ...
        for site, sd in site_data.items():
            ...
            if last == 0 and prev and isinstance(prev,(int,float)) and prev > 500:
                issues.append({
                    ...
                    "severity": "error",
                    "site": site,
                    "message": f"Tombe à 0 alors que {pm}={_fmtn(prev)} — feed absent?",
                    "values": f"{site}: {pm}={_fmtn(prev)} → {lm}=0",
                })
...
def analyse_all_tables(raw_bytes_dict):
    """
    Pour chaque sheet et section, analyse le type de table (volume vs taux)
    et génère des issues QC contextuelles.
    """
    results = []
    ...
    for role, data in fb.items():
        ...
        for sn in wb.sheetnames:
            ...
            for sec_idx, d in enumerate(sections):
                ...
                site_rows = []
                for site, sd in d.items():
                    if not isinstance(site, str) or site.startswith("_") or site in SKIP: continue
                    last = sd.get("last"); prev = sd.get("prev")
                    ...
                    if table_type == "taux":
                        ms = None  # pas de market share pour taux
                    else:
                        ms = (last/dedup*100) if (last and dedup and isinstance(dedup,(int,float)) and dedup > 100) else None

                    status = "ok"
                    if table_type == "volume":
                        if evol is not None and (evol <= -20 or evol >= 40): status = "alert"
                        elif evol is not None and (evol <= -10 or evol >= 25): status = "warn"
                        if ms and ms > 100.5: status = "alert"
                    else:  # taux
                        if isinstance(last,(int,float)) and last > 1.05: status = "alert"
                        elif evol is not None and abs(evol) > 50: status = "warn"

                    site_rows.append({
                        "site": site, "last": last, "prev": prev,
                        "evol": evol, "ms": ms, "status": status,
                ...
                results.append({
                    "file": fname, "sheet": sn, "sec_idx": sec_idx,
                    "label": label, "lm": lm, "pm": pm,
                    "table_type": table_type,
                    "dedup": dedup, "total": total,
                    "sites": site_rows, "issues": issues,
                    "n_error":  sum(1 for i in issues if i["severity"] == "error"),
                    "n_warn":   sum(1 for i in issues if i["severity"] == "warning"),
                    "n_alert":  sum(1 for r in site_rows if r["status"] == "alert"),
                    "n_warn_s": sum(1 for r in site_rows if r["status"] == "warn"),
                })
...
with tab1:
    ...
    with st.expander("🎨 How colours are determined", expanded=False):
        st.markdown("""
**Data integrity tab** — cross-file consistency checks:
- 🔴 **Error** — numbers don't match between files or sections (e.g. 3.1.1 ≠ 1.2, dedup > total)
- ⚠️ **Warning** — statistically unusual change vs history (Z-score > 3.5)
- ✅ **Passed** — check is consistent

**Monthly trends tab** — M/M-1 evolution per series:
- 🔴 **Critical** — drop ≥ 20%, crash vs 12-month peak (< 60%), or 3-month consecutive decline
- 🟡 **Warning** — decline 10–20%, surge ≥ 30%, or Y/Y anomaly > 30%
- ✅ **OK** — stable

**Table analysis tab** — M vs M-1 per table, per website:
- 🔴 **Severe change** — absolute change ≥ 30% M/M-1 **and** previous month ≥ 100 **and** absolute difference ≥ 50
- 🟡 **Notable change** — absolute change 20–30% M/M-1 (same volume thresholds)
- 🔴 **Share >100% of Total** — a site's listing count exceeds the **Total** row of the same table. The **Total** = sum of all listings across all sites (including out-of-panel sites like Immobilier Notaire). Share is calculated as: `site listings ÷ Total`. Except for file 1 / sheet 1 (total résidentiel = Neuf+Ancien), all other tables are **Ancien only**. A share >100% typically means the site and the Total don't cover the same scope.
- ✅ **OK** — change < 20%, or volume too low to be meaningful

> **Volume thresholds:** Changes on values < 100 or with an absolute difference < 50 are ignored — a drop from 9 to 5 (-44%) is noise, not a real issue.

**Special check tab** — market share M/M-1:
- 🔴 **Alert** — share > 100%, or share shift ≥ 5pp, or listing volume ≥ ±25%
- 🟡 **Review** — share shift 3–5pp, or listing volume ±15–25%
        ...
        for i,row in enumerate(flagged[:12]):
            ...
            with cols3[i%3]:
                ...
                fig=go.Figure(go.Scatter(x=x_vals,y=vals,mode="lines+markers",
                    ...
                    connectgaps=False,  # Don't connect across None gaps
                ...
                st.plotly_chart(fig,use_container_width=True,
                                config={"displayModeBar":False},key=f"sc_{i}_{row['site'][:6]}")
                for flag in row["flags"][:2]: st.caption(f"⚠ {flag}")
    else:
...
with tab4:
    ...
    # ─────────────────────────────────────────
    # SUB-TAB 1 : BY WEBSITE
    # ─────────────────────────────────────────
    with sub1:
        st.markdown("#### Select a website to see all issues")

        # Collecter tous les sites ayant au moins une issue
        site_issue_map = {}  # site → [{table, issue}]
        for t in tables:
            for iss in t["issues"]:
                s = iss["site"]
                if s not in site_issue_map:
                    site_issue_map[s] = []
                site_issue_map[s].append({
                    "table": f"{t['file']} › {t['sheet']}",
                    "section": t["label"],
        ...
        # Aussi collecter les sites avec status alert/warn (même sans issue explicite)
        for t in tables:
            for sr in t["sites"]:
                if sr["status"] in ("alert","warn") and sr["site"] in SITES:
                    s = sr["site"]
                    if s not in site_issue_map:
                        site_issue_map[s] = []

        all_sites_with_issues = sorted(site_issue_map.keys(),
            key=lambda x: (-len(site_issue_map.get(x,[])), x))

        if not all_sites_with_issues:
            st.success("No issues detected for any website.")
        else:
            # Résumé par website
            summary_cols = st.columns(min(4, len(all_sites_with_issues)))
            panel_sites_with_issues = [s for s in all_sites_with_issues if s in SITES]
            for i, site in enumerate(panel_sites_with_issues[:8]):
                n = len(site_issue_map.get(site, []))
                n_err = sum(1 for x in site_issue_map.get(site,[]) if x.get("severity")=="error")
                with summary_cols[i % 4]:
                    st.metric(site, f"{n_err} error{'s' if n_err!=1 else ''}", f"{n} issues total")

            st.divider()

            sel_site = st.selectbox(
                "Website",
                all_sites_with_issues,
                format_func=lambda s: f"{'⭐ ' if s in SITES else ''}{s}"
            )

            site_issues = site_issue_map.get(sel_site, [])

            if not site_issues:
                st.info(f"No explicit issue for **{sel_site}** — check the 'By table' subtab.")
            else:
                # Grouper par table
                from collections import defaultdict
                by_table = defaultdict(list)
                for iss in site_issues:
                    by_table[iss["table"]].append(iss)

                n_err_site = sum(1 for x in site_issues if x.get("severity")=="error")
                n_warn_site = sum(1 for x in site_issues if x.get("severity")=="warning")
                st.markdown(f"**{sel_site}** — {n_err_site} error{'s' if n_err_site!=1 else ''} · {n_warn_site} warning{'s' if n_warn_site!=1 else ''} in {len(by_table)} table{'s' if len(by_table)!=1 else ''}")

                for table_name, table_issues in sorted(by_table.items()):
                    n_e = sum(1 for x in table_issues if x.get("severity")=="error")
                    icon = "❌" if n_e > 0 else "⚠️"
                    with st.expander(f"{icon} {table_name} — {table_issues[0].get('section','')} ({table_issues[0].get('month','')})", expanded=(n_e>0)):
                        ttype = table_issues[0].get("table_type","volume")
                        st.caption(f"Table type: {'📊 Volume (listings)' if ttype=='volume' else '📐 Rate (ratios 0–1)'}")
                        for iss in sorted(table_issues, key=lambda x: 0 if x.get("severity")=="error" else 1):
                            ...
                            badge = {
                                         ...
                                         "MS_OVER_100":"🔴 Share >100% of Total","DEDUP_CHANGE_SEVERE":"🔴 Dedup severe change","DEDUP_CHANGE":"🟡 Dedup notable change","ZERO":"🔴 Unexpected zero",
                                         "SUM_MISMATCH":"⚠️ Sum ≠ Total","TAUX>100%":"🔴 Rate >100%",
                                         "TAUX_JUMP":"🔴 Rate jump M/M-1",
                                         "TAUX_ZERO_TO_VALUE":"🔴 Rate: 0% → value",
                                         "TAUX_VALUE_TO_ZERO":"🔴 Rate: value → 0%",
                                     }.get(itype, f"⚠️ {itype}")
                            if sev == "error":
                                st.error(f"**{badge}** — {msg}")
                            else:
                                st.warning(f"**{badge}** — {msg}")

    # ─────────────────────────────────────────
    # SUB-TAB 2 : BY TABLE
    # ─────────────────────────────────────────
    with sub2:
        st.markdown("#### Select a file and table to inspect")
        ...
        # Sheet selector avec badge
        def _sheet_badge(sn):
            ts = [t for t in file_tables if t["sheet"] == sn]
            n_e = sum(t["n_error"] for t in ts)
            n_w = sum(t["n_warn"] for t in ts)
            if n_e > 0: return f"❌ {sn} ({n_e} err)"
            if n_w > 0: return f"⚠️ {sn} ({n_w} warn)"
            return f"✅ {sn}"

        sheet_options = sorted(set(t["sheet"] for t in file_tables))
        sel_sheet = st.selectbox("📄 Sheet", sheet_options,
        ...
        if len(sheet_tables) > 1:
            def _sec_badge(t):
                ...
                return f"{icon} {lbl}"
            sel_t = st.radio("Section", sheet_tables,
                             format_func=_sec_badge, horizontal=False, key="ta_sec")
        else:
        ...
        if not issues:
            ...
            st.markdown(f"**{n_e} error{'s' if n_e!=1 else ''} · {n_w} warning{'s' if n_w!=1 else ''}**")

            ISSUE_EXPLANATIONS = {
                "CHANGE_SEVERE":  "Change ≥30% M/M-1 (up or down) — investigate regardless of direction.",
                "CHANGE":         "Change 20-30% M/M-1 — check with the team.",
                "MS_OVER_100":         "Site listings > Total — mathematically impossible. Check scope (Ancien only vs Neuf+Ancien).","DEDUP_CHANGE_SEVERE": "Deduplicated total changed ≥30% M/M-1 — significant market movement or data issue.","DEDUP_CHANGE":         "Deduplicated total changed 20-30% M/M-1 — worth checking.",
                "ZERO":           "Value dropped to 0 while M-1 was significant — feed missing?",
                "SUM_MISMATCH":   "Normal gap if Total includes out-of-panel sites. Check if gap >25%.",
                "TAUX>100%":      "A rate cannot exceed 100% — calculation error in source file.",
            ...
            for iss in sorted(issues, key=lambda x: 0 if x["severity"]=="error" else 1):
                ...
                explanation = ISSUE_EXPLANATIONS.get(itype, "")
        ...
        if site_rows:
            ...
            def _fmt_val(v, is_taux):
                if v is None: return "—"
                if is_taux: return f"{v*100:.2f}%"
                return _fmtn(v)

            df_data = []
            for r in sorted(site_rows, key=lambda x: ({"alert":0,"warn":1,"ok":2}.get(x["status"],3), -(x["last"] or 0))):
                status_icon = "🔴" if r["status"]=="alert" else "🟡" if r["status"]=="warn" else "✅"
                row = {
                    "": status_icon,
                    "Website": r["site"],
                    f"{sel_t['lm']}": _fmt_val(r["last"], is_taux),
                    f"{sel_t['pm']}": _fmt_val(r["prev"], is_taux),
                }
                if r.get("evol") is not None:
                    row["M/M-1"] = f"{r['evol']:+.1f}%"
                else:
                    row["M/M-1"] = "—"
                if not is_taux and r.get("ms") is not None:
                    row["Market share"] = f"{r['ms']:.1f}%"
                df_data.append(row)
            ...
            if df_data:
                ...
                st.dataframe(_color(df), use_container_width=True, hide_index=True)

            # Bar chart M vs M-1 (volume uniquement)
            if not is_taux:
                ...
                if len(active) >= 2:
                    ...
                    for i, r in enumerate(active):
                        ...
                        fig.add_trace(go.Bar(
                            name=r["site"],
                            x=[sel_t["pm"], sel_t["lm"]],
                            y=[r["prev"] or 0, r["last"] or 0],
                            marker_color=[clr, clr],
                    ...
                    fig.update_layout(
                        ...
                        yaxis=dict(tickformat=".2s", gridcolor="rgba(180,180,180,.3)"),
                        xaxis=dict(showgrid=False),
                    )
                    st.plotly_chart(fig, use_container_width=True, config={"displayModeBar":False})

    # ─────────────────────────────────────────
    # SUB-TAB 3 : ALL ISSUES
    # ─────────────────────────────────────────
    with sub3:
        st.markdown("#### All detected issues — sorted by severity")
        ...
        for t in tables:
            for iss in t["issues"]:
                all_issues_flat.append({
                    ...
                    "Sheet": t["sheet"],
                    "Table": t["label"],
                    "Table type": t["table_type"].capitalize(),
                    "Website": iss["site"],
                    "Month": t["lm"],
                    "Detail": iss["message"],
                })
        ...
        if not all_issues_flat:
            st.success("No issues detected in any table.")
        else:
            n_err = sum(1 for x in all_issues_flat if "Error" in x["Severity"])
            n_warn = len(all_issues_flat) - n_err

            c1, c2, c3 = st.columns(3)
            c1.metric("🔴 Errors", n_err)
            c2.metric("🟡 Warnings", n_warn)
            c3.metric("Total issues", len(all_issues_flat))

            # Filtres
            fc1, fc2 = st.columns(2)
            with fc1:
                sev_filter = st.multiselect("Severity", ["🔴 Error","🟡 Warning"],
                                             default=["🔴 Error","🟡 Warning"], key="sev_f")
            with fc2:
                site_filter_ta = st.multiselect("Website", sorted(set(x["Website"] for x in all_issues_flat)),
                                                 default=[], placeholder="All", key="site_f_ta")

            filtered = [x for x in all_issues_flat
                        if x["Severity"] in sev_filter
                        and (not site_filter_ta or x["Website"] in site_filter_ta)]

            filtered_sorted = sorted(filtered, key=lambda x: (0 if "Error" in x["Severity"] else 1,
                                                                x["File"], x["Sheet"]))

            df_all = pd.DataFrame(filtered_sorted)
            if not df_all.empty:
                def _color_all(df):
                    def _row(row):
                        s = str(row.get("Severity",""))
                        bg = "#fdecec" if "Error" in s else "#fff4d6"
                        return [f"background-color:{bg}" for _ in row]
                    return df.style.apply(_row, axis=1)
                st.dataframe(_color_all(df_all), use_container_width=True, hide_index=True,
                             height=min(700, 42+35*len(df_all)))
                st.download_button(
                    "⬇ Download issues CSV",
    ...
    # ── Summary metrics ──
    n_tables   = len(tables)
    n_alert_t  = sum(1 for t in tables if t["n_alert"] > 0)
    n_warn_t   = sum(1 for t in tables if t["n_warn"] > 0 and t["n_alert"] == 0)
    n_ok_t     = n_tables - n_alert_t - n_warn_t

    ca, cw, co, cn = st.columns(4)
    ca.metric("🔴 Tables with alerts",  n_alert_t)
    cw.metric("🟡 Tables with warnings", n_warn_t)
    co.metric("✅ Clean tables",         n_ok_t)
    cn.metric("📊 Total sections",       n_tables)

    st.divider()

    # ── File selector ──
    file_options = sorted(set(t["file"] for t in tables))
    sel_file = st.selectbox("📁 File", file_options)
    ...
    if len(sheet_tables) > 1:
        sec_labels = [f"Section {t['sec_idx']+1} — {t['label']}" for t in sheet_tables]
        sel_sec_label = st.radio("Section", sec_labels, horizontal=True)
        sel_idx = sec_labels.index(sel_sec_label)
        sel_table = sheet_tables[sel_idx]
    else:
        sel_table = sheet_tables[0]

    st.markdown(f"#### {sel_table['sheet']} › {sel_table['label']}")
    st.caption(f"Reference month: **{sel_table['lm']}** · vs {sel_table['pm']} · "
               f"{'Dedup: ' + fmt(sel_table['dedup']) if sel_table['dedup'] else 'No dedup found'} · "
               f"{'Total: ' + fmt(sel_table['total']) if sel_table['total'] else ''}")

    # ── Per-site table ──
    site_rows = sel_table["sites"]
    if not site_rows:
        st.info("No site data found in this section.")
    else:
        def _status_badge(s):
            return "🔴 Alert" if s == "alert" else "🟡 Review" if s == "warn" else "✅ OK"

        df_sites = pd.DataFrame([{
            "Status":    _status_badge(r["status"]),
            "Site":      r["site"],
            f"M ({sel_table['lm']})":   fmt(r["last"]),
            f"M-1 ({sel_table['pm']})": fmt(r["prev"]) if r["prev"] is not None else "—",
            "M/M-1":     f"{r['evol']:+.1f}%" if r["evol"] is not None else "—",
            "Mkt share": f"{r['ms']:.1f}%" if r["ms"] is not None else "—",
        } for r in sorted(site_rows, key=lambda x: (
            {"alert":0,"warn":1,"ok":2}[x["status"]],
            -(x["last"] or 0)
        ))])
        ...
        def _color_rows(df):
            def _row(row):
                ...
                bg = "#fdecec" if "🔴" in s else "#fff4d6" if "🟡" in s else "#edf7ed"
                return [f"background-color:{bg}" for _ in row]
            return df.style.apply(_row, axis=1)

        st.dataframe(_color_rows(df_sites), use_container_width=True, hide_index=True)

        # ── Mini sparkline for this section (top 6 active sites) ──
        active_sites = [r for r in site_rows if r["last"] and r["last"] > 50][:6]
        if active_sites and len(active_sites) >= 2:
            st.markdown("##### Volume comparison — M vs M-1")
            fig = go.Figure()
            for r in active_sites:
                if r["last"] is not None and r["prev"] is not None:
                    fig.add_trace(go.Bar(
                        name=r["site"],
                        x=[sel_table["pm"], sel_table["lm"]],
                        y=[r["prev"], r["last"]],
                        text=[fmt(r["prev"]), fmt(r["last"])],
    ...
    # ── All-tables summary view ──
    st.markdown("#### All tables — summary")
    st.caption("Every section across all files. Click a row to navigate (use the selectors above).")

    summary_rows = []
    for t in sorted(tables, key=lambda x: (x["n_alert"]==0, x["n_warn"]==0, x["file"], x["sheet"], x["sec_idx"])):
        status = "🔴 Alert" if t["n_alert"] > 0 else "🟡 Warn" if t["n_warn"] > 0 else "✅ OK"
        n_sites = len(t["sites"])
        top_site = max(t["sites"], key=lambda r: r["last"] or 0, default=None)
        summary_rows.append({
            "Status":  status,
            "File":    t["file"],
            "Sheet":   t["sheet"],
            "Section": t["label"] or f"sec {t['sec_idx']+1}",
            "Month":   t["lm"],
            "Sites":   n_sites,
            "Top site": f"{top_site['site']} ({fmt(top_site['last'])})" if top_site else "—",
            "Dedup":   fmt(t["dedup"]) if t["dedup"] else "—",
            "Alerts":  t["n_alert"],
            "Warnings":t["n_warn"],
        })
    ...
    st.dataframe(df_summary, use_container_width=True, hide_index=True,
                 height=min(600, 42 + 35*len(summary_rows)))

    st.download_button(
        "⬇ Download table analysis CSV",
        df_summary.to_csv(index=False).encode("utf-8-sig"),
        f"table_analysis_{lm_ref}.csv", "text/csv"
    )