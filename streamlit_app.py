...
def classify_generic(raw, file_roles):
    """Generic classify — matches files to roles by keyword."""
    out = {}
    for fname, data in raw.items():
        nl = norm(fname)
        for role, label in file_roles.items():
            ...
            if any(p in nl for p in [f"_{num}_", f"_{num}.", f"-{num}-", f"{num}_"]):
                out[role] = data; break
        else:
            if "evolution" in nl or "panel" in nl or "website" in nl:
                out.setdefault("file1", data)
            elif "qualit" in nl or "perform" in nl or "offer" in nl:
                out.setdefault("file2", data)
            elif "pros" in nl or "agent" in nl:
                out.setdefault("file3", data)
    return out
