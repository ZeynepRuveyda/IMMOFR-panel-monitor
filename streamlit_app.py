...
tab1,tab2,tab3,tab4,tab_ms=st.tabs(["📋  Overview","📈  Trends — Monthly · Yearly · Dept · Région","🔍  Data integrity","📊  Table analysis","🎯  Special check for LBC"])
    ...
    if "file1" in fb:
        try:
            ...
            if ws11:
                ...
                c5.metric("Website trend alerts (M/M-1)",
                          f"🔴 {n_alert}  🟡 {n_wtr}",
...
<tbody>{rows_html}</tbody>
    ...
    with subtab_m:
        def _is_mom_flag(f, pm):
            """Sadece M/M-1 bazlı flaglar — Drop/Surge/Decline vs M-1."""
            fl = f.lower()
            return (("drop" in fl or "surge" in fl or "decline" in fl)
                    and ("12m" not in fl and "peak" not in fl and "y-1" not in fl
                         and "downtrend" not in fl and "year" not in fl))

        flagged_m = []
        seen_m = set()
        for r in tr_site:
            mom_flags = [f for f in (r.get("flags") or [])
                         if _is_mom_flag(f, r.get("pm",""))]
            # Status: sadece M/M-1 flag'i varsa bu tab'da göster
            if mom_flags:
                mom_status = "alert" if any("drop" in f.lower() and float(f.split("%")[0].split()[-1].replace("+","")) <= -20
                                            or "surge" in f.lower()
                                            for f in mom_flags) else "warn"
                # Daha basit: orijinal status'u koru ama sadece mom_flag'i olanları dahil et
        ...
        st.caption(f"**Website-level series** — 🔴 {n_crit_m} critical · 🟡 {n_warn_m} warnings (unique site+sheet combinations)")
        ...
        with st.expander("📋 Detailed table — click to expand for in-depth review", expanded=False):
            st.caption("Filter by severity. Hover over ⚠ note for full details.")
            sev_m = st.radio("Show", ["🔴 Critical", "🟡 Warnings", "🔴🟡 Both"],
                             index=2, horizontal=True, key="sev_m_tbl")
            tr_m_show = [r for r in tr_site if
                         (sev_m=="🔴🟡 Both") or
                         (sev_m=="🔴 Critical" and r["status"]=="alert") or
                         (sev_m=="🟡 Warnings" and r["status"]=="warn")]
            # tr_m_show için flags'leri sadece M/M-1 olanlarla override et
            tr_m_filtered = [{**r, "flags": [f for f in (r.get("flags") or [])
                              if _is_mom_flag(f, r.get("pm",""))]} for r in tr_m_show]
            html_m = _trend_table_html(tr_m_filtered)
            st.markdown(html_m, unsafe_allow_html=True)
            if tr_m_filtered:
                df_dl = pd.DataFrame([{
                    "Site": r["site"], "Table": r.get("section","") or r.get("sheet",""),
                    f"M ({r['lm']})": _fmt_full(r["lv"]), f"M-1 ({r['pm']})": _fmt_full(r["pv"]),
                    "M/M-1": f"{r['evol']:+.1f}%" if r["evol"] is not None else "—",
                    "Notes": " · ".join(r.get("flags") or []),
                } for r in tr_m_filtered])
                st.download_button("⬇ Download", df_dl.to_csv(index=False).encode("utf-8-sig"),
    ...
    with subtab_y:
        ...
        n_y1 = sum(1 for r in tr_y_base if r.get("evol_y1") is not None)
        n_crit_y = sum(1 for r in flagged_y if r["status"]=="alert")
        n_warn_y = sum(1 for r in flagged_y if r["status"]=="warn")
        st.caption(f"**Y-1 data for {n_y1} series** — 🔴 {n_crit_y} critical · 🟡 {n_warn_y} warnings (unique site+sheet)")
        ...
        with st.expander("📋 Detailed table — click to expand for in-depth review", expanded=False):
            ...
            tr_y_filt = [r for r in tr_y_show if
                         (sev_y=="🔴🟡 Both") or
                         (sev_y=="🔴 Critical" and r["status"]=="alert") or
                         (sev_y=="🟡 Warnings" and r["status"]=="warn")]
            html_y = _trend_table_html(tr_y_filt, cols_extra="y1")
            st.markdown(html_y, unsafe_allow_html=True)
            if tr_y_filt:
                df_dl_y = pd.DataFrame([{
                    "Site": r["site"], "Table": r.get("section","") or r.get("sheet",""),
                    f"M ({r['lm']})": fmt(r["lv"]),
                    "M/Y-1": f"{r.get('evol_y1'):+.1f}%" if r.get("evol_y1") is not None else "—",
                    "Notes": " · ".join(r.get("flags") or []),
                } for r in tr_y_filt])
                st.download_button("⬇ Download", df_dl_y.to_csv(index=False).encode("utf-8-sig"),
    ...
    with subtab_d:
        ...
        st.caption(f"**Department-level series (M/M-1)** — 🔴 {n_crit_d} critical · 🟡 {n_warn_d} warnings (unique site+sheet combinations)")
        ...
        with st.expander("📋 Detailed table — click to expand for in-depth review", expanded=False):
            ...
            for r in rows_to_show_d:
                mom_notes = [f for f in (r.get("flags") or []) if "Y-1" not in f and "year" not in f.lower()]
                df_d_rows.append({
            ...
            if df_d_rows:
                ...
                st.dataframe(df_d, use_container_width=True, hide_index=True,
                             ...
                             column_config={"ℹ️": st.column_config.TextColumn("ℹ️", width=40,
                                            help="Hover to see note")})
        ...
        for r in tr_region:
            if r["status"] in ("alert","warn") and r.get("flags"):
                ...
                if mom_flags:
                    ...
                    if k not in seen_r:
                        seen_r.add(k)
                        flagged_r.append({**r, "flags": mom_flags})

        n_crit_r = sum(1 for r in flagged_r if r["status"]=="alert")
        n_warn_r = sum(1 for r in flagged_r if r["status"]=="warn")
        st.caption(f"**Région-level series (M/M-1)** — 🔴 {n_crit_r} critical · 🟡 {n_warn_r} warnings (unique région+sheet)")
        ...
        if all_issues_flat:
            ...
            if not df_all.empty:
                ...
                st.dataframe(_ca(df_all),use_container_width=True,hide_index=True,
                             ...
                             column_config={"ℹ️": st.column_config.TextColumn("ℹ️", width=40, help="Hover to see detail")})
