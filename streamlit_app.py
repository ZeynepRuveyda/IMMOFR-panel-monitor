...
def classify_mom_status(raw_vals, li):
    ...
    if pv is not None and (not isinstance(pv, (int, float)) or pv <= 0):
        pv = None
    status, reasons = "ok", []
    evol = (lv/pv - 1) * 100 if (lv is not None and isinstance(lv, (int, float)) and pv) else None
    if evol is not None:
        if evol <= -20:
    ...
    if hist_peak and lv is not None and isinstance(lv, (int, float)) and lv > 0 and lv/max(hist_peak) < 0.6:
        status = "alert"; crash_pct = (lv/max(hist_peak) - 1) * 100; reasons.append("crash")
    downtrend_pct = None
    if li >= 2:
        ...
        for k,v in d.items():
            if isinstance(k,str) and "dédupliqué marché" in k.lower() and not k.startswith("_"):
                ...
                if len(dvals)>=2:
                    ...
                    elif dcl["reasons"] and ("decline" in dcl["reasons"] or "surge" in dcl["reasons"]):
                        issues.append({"type":"DEDUP_CHANGE","severity":"warning","site":dl,
                            "message":f"Deduplicated total: {dcl['evol']:+.1f}% — {pm}: {_fmtn(dp_val)} → {lm}: {_fmtn(dv)}","values":""})
                break
    return issues
    ...
    for role,data in fb.items():
        ...
        for sn in wb.sheetnames:
            ...
            for sec_idx,d in enumerate(sections):
                if not d or len(d)<=3:
                    results.append({"file":fname,"sheet":sn,"sec_idx":sec_idx,
...
n_wtr   = len({f"{r['site']}_{r['sheet']}" for r in _tr_sites_only if r["status"]=="warn"})
...
with tab1:
    if n_err>0:
        st.error(
            ...
            f"Website trends (M/M-1): {n_alert} critical · {n_wtr} warnings"
        )
        ...
        st.warning(
            ...
            f"Website trends (M/M-1): {n_alert} critical series · {n_wtr} warnings  \n"
    ...
    if "file1" in fb:
        try:
            ...
            if ws11:
                ...
                c5.metric("Trend alerts (M/M-1)",
                          f"🔴 {n_alert} · 🟡 {n_wtr}",
                          help="Critical and warning series at website level only. Dept/région trends visible in Trends tab.")
            wb1.close()
        except: pass
        ...
        if not attention_df.empty:
            ...
            st.dataframe(
                ...
                column_config={
                    ...
                    "Website":      st.column_config.TextColumn(width=130),
                    "Segment":      st.column_config.TextColumn(width=180),
                    "Listings MoM": st.column_config.TextColumn(width=110),
                    "Share Δ":      st.column_config.TextColumn(width=90),
                    "ℹ️": st.column_config.TextColumn("ℹ️", width=40, help="Hover to see reason"),
                },
            )
        else:
            st.divider()
        ...
        if not df_site.empty:
            st.dataframe(
                ...
                column_config={
                    ...
                    "Listings MoM":      st.column_config.TextColumn(width=110),
                    "Share Δ":           st.column_config.TextColumn(width=90),
                    "Dedup denominator": st.column_config.TextColumn(width=140),
                    "Dedup MoM":         st.column_config.TextColumn(width=100),
                    "ℹ️": st.column_config.TextColumn("ℹ️", width=40, help="Hover to see reason"),
                },
            )
        else:
...
with tab2:
    ...
    def _draw_flagged(flagged, key_prefix):
        """Grafik filtresi + sparkline grid."""
        n_crit = sum(1 for r in flagged if r["status"]=="alert")
        n_warn = sum(1 for r in flagged if r["status"]=="warn")

        # Filtre radio
        show_filter = st.radio(
            "Show",
            ["🔴 Critical only", "🟡 Warnings only", "🔴🟡 Both"],
            index=2, horizontal=True, key=f"{key_prefix}_show",
        )
        if "Critical" in show_filter:
            flagged = [r for r in flagged if r["status"]=="alert"]
        elif "Warnings" in show_filter:
            flagged = [r for r in flagged if r["status"]=="warn"]

        if not flagged:
            st.info("No series match this filter.")
            return

        st.caption(f"Showing {len(flagged)} series — 🔴 {n_crit} critical · 🟡 {n_warn} warnings")
        cols3 = st.columns(3)
        for i, row in enumerate(flagged[:18]):
            ...
            with cols3[i % 3]:
                ...
                # Show section if it adds info, otherwise just sheet
                if section_lbl and section_lbl != sheet_lbl and len(section_lbl) > 2:
                    full_label = f"{section_lbl}  \n*{sheet_lbl}*"
                else:
                    full_label = f"*{sheet_lbl}*"
                label = row.get("site","")
    ...
    def _trend_table_html(rows, title="All series — M/M-1", cols_extra=None):
        ...
        for r in sorted_rows:
            icon = "🔴" if r["status"]=="alert" else "🟡" if r["status"]=="warn" else "⚪" if r["status"]=="inactive" else "✅"
            bg = "#fff0f0" if r["status"]=="alert" else "#fffbe6" if r["status"]=="warn" else "transparent"
...
<tbody>{rows_html}</tbody>
    ...
    with subtab_m:
        ...
        n_warn_m = sum(1 for r in flagged_m if r["status"]=="warn")
        st.caption(f"**Website-level series** — 🔴 {n_crit_m} critical · 🟡 {n_warn_m} warnings"
                   f"  \n*Critical = M/M-1 drop ≥ 20% · Warning = drop 10-20% or surge ≥ 30%*"
        ...
        with st.expander("📋 Detailed table — click to expand for in-depth review", expanded=False):
            ...
            sev_m = st.radio("Show", ["🔴 Critical", "🟡 Warnings", "🔴🟡 Both"],
                             index=2, horizontal=True, key="sev_m_tbl")
            tr_m_show = [r for r in flagged_m if
                         (sev_m=="🔴🟡 Both") or
                         (sev_m=="🔴 Critical" and r["status"]=="alert") or
                         (sev_m=="🟡 Warnings" and r["status"]=="warn")]

            if tr_m_show:
                lm_ = tr_m_show[0]["lm"]; pm_ = tr_m_show[0]["pm"]
                df_m_tbl = pd.DataFrame([{
                    "Status": "🔴" if r["status"]=="alert" else "🟡",
                    "Website": r["site"],
                ...
                def _color_m(df):
                    def _row(row):
                        ...
                        bg = "#fdecec" if s=="🔴" else "#fff4d6" if s=="🟡" else "transparent"
    ...
    with subtab_d:
        ...
        n_warn_d = sum(1 for r in flagged_d if r["status"]=="warn")
        st.caption(f"**Department-level series (M/M-1)** — 🔴 {n_crit_d} critical · 🟡 {n_warn_d} warnings"
                   f"  \n*Critical = M/M-1 drop ≥20% · Warning = drop 10-20% or surge ≥30%*")
        ...
        with st.expander("📋 Detailed table — click to expand for in-depth review", expanded=False):
            ...
            sev_d = st.radio("Show", ["🔴 Critical", "🟡 Warnings", "🔴🟡 Both"],
                             index=2, horizontal=True, key="sev_d_tbl")

            rows_to_show_d = [r for r in flagged_d if
                              (sev_d == "🔴🟡 Both") or
                              (sev_d == "🔴 Critical" and r["status"] == "alert") or
                              (sev_d == "🟡 Warnings" and r["status"] == "warn")]
    ...
    with subtab_r:
        ...
        n_warn_r = sum(1 for r in flagged_r if r["status"]=="warn")
        st.caption(f"**Région-level series (M/M-1)** — 🔴 {n_crit_r} critical · 🟡 {n_warn_r} warnings"
                   f"  \n*Critical = M/M-1 drop ≥20% · Warning = drop 10-20% or surge ≥30%*")
        ...
        with st.expander("📋 Detailed table — click to expand for in-depth review", expanded=False):
            ...
            # Severity filter
            sev_r = st.radio("Show", ["🔴 Critical", "🟡 Warnings", "🔴🟡 Both"],
                             index=2, horizontal=True, key="sev_r_tbl")

            rows_to_show = [r for r in flagged_r if
                            (sev_r == "🔴🟡 Both") or
                            (sev_r == "🔴 Critical" and r["status"] == "alert") or
                            (sev_r == "🟡 Warnings" and r["status"] == "warn")]
...
with tab_dq:
    ...
    st.caption(
        ...
        "Every table/section found in the uploaded files is listed below, including clean ones."
    )
    st.divider()
    ...
    BADGES={
        "CHANGE_SEVERE":"🔴 M/M-1 drop ≥20%","CHANGE":"🟡 M/M-1 decline/surge",
        ...
        "DEDUP_CHANGE_SEVERE":"🔴 Dedup drop ≥20%","DEDUP_CHANGE":"🟡 Dedup decline/surge",
    }
    EXPL={
        "CHANGE_SEVERE":"M/M-1 drop ≥20% — same threshold as a 🔴 alert in the Trends tab.",
        "CHANGE":"M/M-1 decline 10–20% or surge ≥30% — same threshold as a 🟡 warning in the Trends tab.",
        ...
        "DEDUP_CHANGE":"Deduplicated total decline/surge M/M-1 — same threshold as Trends.",
    }
    TTYPE_LBL = {
    ...
    tables_by_g=defaultdict(list)
    for t in tables_show:
        grp=t["file"].split("—")[0].strip()
        tables_by_g[grp].append(t)

    group_order = {g:i for i,g in enumerate(GROUP_INFO.keys())}
    all_groups = sorted(set(by_g)|set(tables_by_g), key=lambda g: group_order.get(g,99))

    for grp in all_groups:
        items=by_g.get(grp,[])
        grp_tables=tables_by_g.get(grp,[])
        ne=sum(1 for c in items if not c["ok"] and c["sev"]=="error")
        nw=sum(1 for c in items if c["sev"]=="warning")
        te=sum(1 for t in grp_tables if t["n_error"]>0)
        tw=sum(1 for t in grp_tables if t["n_warn"]>0 and t["n_error"]==0)
        total_err, total_warn = ne+te, nw+tw
        title,sub=GROUP_INFO.get(grp,(f"Group {grp}",""))
        badge=(f"{total_err} error{'s' if total_err!=1 else ''}" if total_err else
               f"{total_warn} warning{'s' if total_warn!=1 else ''}" if total_warn else "all clear")
        icon="❌" if total_err else "⚠️" if total_warn else "✅"
        with st.expander(f"{icon}  {title} — {badge}", expanded=(total_err>0)):
            st.caption(sub)
            if items:
                st.markdown("**Formula checks** (cross-file consistency)")
            ...
            if grp_tables:
                if items: st.divider()
                st.markdown(f"**Table anomalies** — {len(grp_tables)} table{'s' if len(grp_tables)!=1 else ''} checked")
                ordered_t=sorted(grp_tables, key=lambda t:(0 if t["n_error"] else 1 if t["n_warn"] else 2, t["sheet"], t["sec_idx"]))
                for t in ordered_t:
                    ticon="❌" if t["n_error"] else "⚠️" if t["n_warn"] else "✅"
                    lbl=t["label"] or f"Section {t['sec_idx']+1}"
                    n_iss=len(t["issues"])
                    sub_badge=f"{n_iss} issue{'s' if n_iss!=1 else ''}" if n_iss else "clean"
                    with st.expander(f"{ticon} {t['sheet']} › {lbl} — {sub_badge}", expanded=False):
                        cc1,cc2,cc3=st.columns(3)
                        cc1.caption(f"Month: **{t['lm']}** vs {t['pm']}")
                        cc2.caption(TTYPE_LBL.get(t["table_type"],t["table_type"]))
                        cc3.caption(f"Market dedup: {_fmtn(t['dedup']) if t['dedup'] else '—'}")
                        if not t["issues"]:
                            st.success("✅ No anomalies detected.")
                        else:
                            for iss in sorted(t["issues"], key=lambda x:0 if x["severity"]=="error" else 1):
                        ...
                        if t["sites"]:
                            ...
                            df_data=pd.DataFrame([{
                                ...
                                "Website": r["site"],
                                f"M ({t['lm']})": _fv(r["last"]),
                                f"M-1 ({t['pm']})": _fv(r["prev"]),
                                "M/M-1": f"{r['evol']:+.1f}%" if r["evol"] is not None else "—",
                                **({} if is_taux else {"Market share": f"{r['ms']:.1f}%" if r.get("ms") else "—"}),
                            } for r in sorted(t["sites"], key=lambda x:({"alert":0,"warn":1,"ok":2}[x["status"]],-(x["last"] or 0)))])
                            def _cr(df):
                                def _row(row):
                                    s=str(row.get("",""))
    ...
    # ── Combined export ──
    rows_exp=[{"Type":"Formula check","Category":GROUP_INFO.get(c["group"],("",""))[0],"Check":c["name"],
               "Result":"❌ Error" if not c["ok"] and c["sev"]=="error"
                        else "⚠️ Warning" if c["sev"]=="warning" else "✅ OK",
               "Detail":c["detail"]} for c in checks]
    for t in tables:
        if not t["issues"]:
            rows_exp.append({"Type":"Table anomaly","Category":t["file"],
                              "Check":f"{t['sheet']} › {t['label']}",
                              "Result":"✅ OK","Detail":"No anomalies detected"})
        for iss in t["issues"]:
            rows_exp.append({"Type":"Table anomaly","Category":t["file"],
                              "Check":f"{t['sheet']} › {t['label']} — {iss['site']}",
                              "Result":"❌ Error" if iss["severity"]=="error" else "⚠️ Warning",
                              "Detail":iss["message"]})
    st.download_button("⬇ Download full report (formula checks + table anomalies)",
                       pd.DataFrame(rows_exp).to_csv(index=False).encode("utf-8-sig"),
                       f"data_table_checks_{lm_ref}.csv","text/csv")
