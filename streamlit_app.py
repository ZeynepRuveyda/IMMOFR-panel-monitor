...
<tbody>{rows_html}</tbody>
    ...
    with subtab_m:
        ...
        html_m = _trend_table_html(tr_site)
    ...
    with subtab_y:
        ...
        if tr_y_show:
            html_y = _trend_table_html(tr_y_show, cols_extra="y1")
    ...
    with subtab_d:
        ...
        for r in flagged_d:
            ...
            df_d_rows.append({
                "Status": "🔴" if r["status"]=="alert" else "🟡" if r["status"]=="warn" else "✅",
                "Département": r["site"],
                f"M ({r['lm']})":   fmt(r["lv"]),
                f"M-1 ({r['pm']})": fmt(r["pv"]),
                "M/M-1": f"{r['evol']:+.1f}%" if r["evol"] is not None else "—",
                "Notes": " · ".join(mom_notes[:2]) if mom_notes else "—",
            })
    ...
    with subtab_r:
        ...
        st.divider()
        st.markdown("#### All région series — M/M-1")

        # Tabloyu flagged_r'dan üret — KPI ile tutarlı
        df_r_rows = []
        for r in flagged_r:
            mom_notes = [f for f in (r.get("flags") or []) if "Y-1" not in f and "year" not in f.lower()]
            df_r_rows.append({
        ...
        if df_r_rows:
            ...
            st.download_button("⬇ Download", df_r.to_csv(index=False).encode("utf-8-sig"),
                               f"region_{lm_ref}.csv", "text/csv")
        else:
            st.info("No région data found. Select a geographic sheet (3.2, 4.1...) above.")
