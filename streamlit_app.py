...
tab1,tab_ms,tab2,tab3,tab4=st.tabs(["📋  Overview","🎯  Special check","📈  Monthly/Yearly Trends","🔍  Data integrity","📊  Table analysis"])
...
with tab2:
    ...
    selected_sheets_geo = [s for s in (sheet_sel or all_sheets_tr) if is_geo_sheet(s)]
    is_geo_mode = len(selected_sheets_geo) > 0 and len(sheet_sel) > 0

    # ── Website seçici ──
    all_sites_tr = sorted(set(r["site"] for r in tr_sheet_filtered if r["status"]!="inactive"))

    # Geo modda: siteler = Région/Département adları; normal modda: site adları
    # Sadece geo modda "Department" label'ı kullan
    ...
    # Geo modda ve website seçildiyse → département seçici de göster
    if is_geo_mode and sel_site_tr != "All":
        # Bu site için mevcut departmanlar (sub-rows)
        depts_available = sorted(set(r.get("section","") for r in tr_filtered if r.get("section","")))
        if depts_available:
            sel_dept = st.selectbox("Department (sub-area)", ["All"] + depts_available, key="tr_dept")
            if sel_dept != "All":
