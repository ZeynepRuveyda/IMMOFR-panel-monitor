...
with tab2:
    ...
    with subtab_y:
        ...
        df_y = _trend_table(tr_y_base, cols_extra="y1")
    ...
    with subtab_d:
        ...
        for r in tr_dept:
            site = r["site"]
            if site not in dept_summary:
                dept_summary[site] = r
            else:
                order = {"alert":0,"warn":1,"ok":2,"inactive":3}
                if order.get(r["status"],3) < order.get(dept_summary[site]["status"],3):
                    dept_summary[site] = r

        df_d_rows = []
        for site, r in sorted(dept_summary.items(),
                               key=lambda x: {"alert":0,"warn":1,"ok":2,"inactive":3}.get(x[1]["status"],3)):
    ...
    with subtab_r:
        ...
        # Région bazında grupla — her région için en kötü durumu göster
        region_summary = {}
        for r in tr_region:
            site = r["site"]
            if site not in region_summary:
                region_summary[site] = r
            else:
                # En kötü status'u tut
                existing = region_summary[site]
                order = {"alert": 0, "warn": 1, "ok": 2, "inactive": 3}
                if order.get(r["status"], 3) < order.get(existing["status"], 3):
                    region_summary[site] = r

        df_r_rows = []
        for site, r in sorted(region_summary.items(),
                               key=lambda x: {"alert":0,"warn":1,"ok":2,"inactive":3}.get(x[1]["status"],3)):
            # M/M-1 notlarını filtrele (Y-1 notları hariç)
            mom_notes = [f for f in (r.get("flags") or []) if "Y-1" not in f and "year" not in f.lower()]
            df_r_rows.append({
                "Status": "🔴" if r["status"]=="alert" else "🟡" if r["status"]=="warn" else "✅",
                "Région": site,
